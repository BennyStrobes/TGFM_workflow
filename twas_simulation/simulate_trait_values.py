...
def simulate_non_mediated_variant_causal_effect_sizes_old(variant_annotation_file, real_sldsc_results_file, per_element_heritability, total_heritability, fraction_expression_mediated_heritability, non_mediated_causal_effects_output):
...
def simulate_non_mediated_variant_causal_effect_sizes(simulated_expression_summary_file, per_element_heritability, n_non_mediated_variants_per_gene, non_mediated_causal_effects_output, genotype_bim):
	variant_names = []
	variant_name_to_position = {}
	f = open(genotype_bim)
	counter = 0
	for line in f:
		line = line.rstrip()
		data = line.split('\t')
		rsid = data[1]
		variant_names.append(rsid)
		variant_name_to_position[rsid] = counter
		counter = counter +1
	f.close()
	variant_names = np.asarray(variant_names)

	# initialize array of causal effect sizes
	causal_non_mediated_effect_sizes = np.zeros(len(variant_names))
	...
	for line in f:
		...
		if head_count == 0:
			...
			continue
		ensamble_id = data[0]
		cis_snp_position_file = data[5]
		cis_snp_positions = np.load(cis_snp_position_file)

		cis_variants = variant_names[cis_snp_positions]

		# Quick error checking
		cis_variants2 = np.load(data[4], allow_pickle=True)
		if np.array_equal(cis_variants, cis_variants2[:,1]) == False:
			print('assumptino erororo')
			pdb.set_trace()

		n_cis_variants = len(cis_variants)

		causal_variants = np.random.choice(cis_variants, size=int(n_non_mediated_variants_per_gene), replace=False)
		causal_variant_effect_sizes = np.random.normal(loc=0.0, scale=np.sqrt(per_element_heritability),size=int(n_non_mediated_variants_per_gene))

		for cv_iter, cv_name in enumerate(causal_variants):
			cv_global_index = variant_name_to_position[cv_name]
			causal_non_mediated_effect_sizes[cv_global_index] = causal_non_mediated_effect_sizes[cv_global_index] + causal_variant_effect_sizes[cv_iter]

	f.close()

	# Print to output file
	t = open(non_mediated_causal_effects_output,'w')
	for var_iter in range(len(variant_names)):
		t.write(variant_names[var_iter] + '\t' + str(causal_non_mediated_effect_sizes[var_iter]) + '\n')
	t.close()
...
n_non_mediated_variants_per_gene = float(sys.argv[8])
fraction_causal_genes = float(sys.argv[9])
