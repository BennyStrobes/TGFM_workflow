...
def overlap_ukbb_components_with_epimap_in_a_chromosome(ukbb_study, ukbb_study_component_file, chrom_string, epimap_chromosome, epimap_cell_types, t_dicti):
	...
	if chrom_string == 'chr1':
		t_dicti[ukbb_study].write('chrom_num\tlead_variant_position\tlead_variant_name\tlead_variant_pi\twindow_id\tcs_variants\tcs_pi\tmax_un_normalized_cell_type_weight')
	...
	for line in f:
		...
		if np.sum(un_normalized_assignments) > 0:
			...
			max_cell_type_weight = np.max(normalized_assignments)
		else:
...
epimap_input_dir = sys.argv[2]
epimap_ukbb_overlap_dir = sys.argv[3]
correlation_threshold = float(sys.argv[4])
...
for ukbb_study in ukbb_studies:
	study_output_file = epimap_ukbb_overlap_dir + ukbb_study + '_component_overlap_with_epimap_' + str(correlation_threshold) + '.txt'
