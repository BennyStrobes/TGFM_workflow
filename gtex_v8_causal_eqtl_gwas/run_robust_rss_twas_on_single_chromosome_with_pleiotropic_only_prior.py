...
pleiotropic_variance_file = output_dir + trait_name + '_' + gene_version + '_count_genes_once_null_init_only_robust_pleiotropic_prior_precision_temp.txt'
...
# open output file
output_file = output_dir + trait_name + '_' + chrom_num + '_summary_tgfm_robust_results_' + gene_version + '_pleiotropic_only_prior.txt'
...
for line in f:
	...
	if pickled_data_file == 'NA':
		...
		twas_obj = tgfm_robust_causal_twas.TGFM_CAUSAL_TWAS(estimate_prior_variance=False,  prior_variance=1000000.0, pleiotropic_prior_variance=pleiotropic_variance, convergence_thresh=1e-6)
		...
		tgfm_twas_pkl_file = output_dir + trait_name + '_' + component_name + '_' + gene_version + '_tgfm_robust_twas_pleiotropic_only_prior_results.pkl'
		...
		# TGFM RSS Regression
		tgfm_rss_regression_fm_results_table_file = output_dir + trait_name + '_' + component_name + '_' + gene_version + '_tgfm_robust_rss_regression_fine_mapping_pleiotropic_only_prior_table.txt'
		tgfm_rss_fm_obj_regr.posterior_prob_df.to_csv(tgfm_rss_regression_fm_results_table_file, sep='\t', index=False)
		tgfm_rss_regression_tissue_fm_results_table_file = output_dir + trait_name + '_' + component_name + '_' + gene_version + '_tgfm_robust_rss_regression_tissue_fine_mapping_pleiotropic_only_prior_table.txt'
