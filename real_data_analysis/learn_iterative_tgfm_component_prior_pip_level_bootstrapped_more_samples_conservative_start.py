...
def extract_pseudotissue_names(gtex_pseudotissue_file, ignore_tissues,ignore_testis=False):
...
def learn_iterative_variant_gene_tissue_prior_pip_level_bootstrapped(component_level_abf_summary_file, tgfm_version, tissue_names, per_window_abf_output_stem, max_iter=60, n_bootstraps=200, random_bootstrap=True, version='v1'):
	...
	# Initialize prior probability emperical distribution
	variant_prob_distr = np.ones(n_bootstraps)*.1
	tissue_probs_distr = np.ones((len(tissue_names), n_bootstraps))*.01
...
num_jobs=8
concatenated_pip_summary_file = new_tgfm_stem + '_iterative_prior_m_iter2_' + suffix
...
###################################################
component_level_abf_summary_file = new_tgfm_stem + '_iterative_prior_v2_m_iter2' + '_tgfm_component_level_abf_summary.txt'
per_window_abf_output_stem = new_tgfm_stem + '_iterative_prior_v2_m_iter2_per_window_abf_'
...
# Print to output
variant_gene_distr_prior_output_file = perm_iterative_prior_results + '_m_iter2_iterative_variant_gene_prior_v2_pip_level_bootstrapped.txt'
...
variant_gene_counts_output_file = perm_iterative_prior_results + '_m_iter2_iterative_variant_gene_counts_pip_level_bootstrapped.txt'
