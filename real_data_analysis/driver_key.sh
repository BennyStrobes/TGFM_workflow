...
# See "Reference" on https://www.gtexportal.org/home/datasets
...
non_disease_specific_gene_sets_file="/n/groups/price/martin/LDSPEC_data/gene_annotation/sam_gene_annotation/geneset_list_curated.csv"
...
#ukbb_in_sample_ld_dir="/n/scratch3/users/j/jz286/imp_geno_chimp.gdreg_ld_1e7/"
#ukbb_in_sample_genotype_dir="/n/scratch3/users/j/jz286/imp_geno_chimp/"
...
output_root="/n/scratch/users/b/bes710/causal_eqtl_gwas/gtex/"
...
# Directory containing preprocessed sc TGFM data
preprocessed_sc_pb_tgfm_data_dir=$output_root"preprocessed_sc_pb_tgfm_data/"
...
if false; then
original_tissue_name="Whole_Blood"
tissue_name="Whole_Blood_subsampled"
subsample_size="113"
sh preprocess_subsampled_gtex_expression_for_gene_model_analysis_in_single_tissue.sh $original_tissue_name $tissue_name $xt_pc_gene_list_file $gtex_expression_dir $gtex_covariate_dir $gtex_genotype_dir $gtex_processed_expression_dir $gtex_processed_genotype_dir $ukbb_preprocessed_for_genome_wide_susie_dir $subsample_size
fi
...
composit_tissue_string="Whole_Blood_subsampled"
if false; then
sh create_pseudotissue_gene_model_input_summary_file.sh $pseudotissue_name $composit_tissue_string $gtex_processed_expression_dir $gtex_pseudotissue_gene_model_input_dir $num_jobs
fi
...
if false; then
sed 1d $trait_missing_tissue_file | while read trait_name study_file sample_size h2 ignore_tissues; do
	for job_number in $(seq 0 $(($num_jobs-1))); do
		tgfm_input_summary_file=${preprocessed_tgfm_data_dir}${gene_type}"_tgfm_input_data_summary.txt"
		tgfm_output_stem=${tgfm_results_dir}"tgfm_results_"${trait_name}"_"${gene_type}"_ignore_"${ignore_tissues}
		sbatch run_tgfm_shell.sh $trait_name $tgfm_input_summary_file $tgfm_output_stem $gtex_pseudotissue_file $job_number $num_jobs $ignore_tissues
...
#################################
# Run TGFM-sampler when holding out some of the presumed causal tissues
#################################
if false; then
gene_type="component_gene"
num_jobs="8"
...
num_jobs="8"
if false; then
trait_missing_tissue_file=$ukbb_sumstats_hg38_dir"ukbb_hg38_sumstat_files_with_samp_size_and_h2_readable3_throw_out_tissue_ignore_cholesterol_and_monocyte.txt"
sed 1d $trait_missing_tissue_file | while read trait_name study_file sample_size h2 ignore_tissues; do
	for job_number in $(seq 0 $(($num_jobs-1))); do
		tgfm_input_summary_file=${preprocessed_tgfm_data_dir}${gene_type}"_tgfm_input_data_summary.txt"
		tgfm_output_stem=${tgfm_results_dir}"tgfm_results_"${trait_name}"_"${gene_type}"_ignore_"${ignore_tissues}
...
#################################
tgfm_input_summary_file=${preprocessed_tgfm_data_dir}${gene_type}"_tgfm_input_data_summary.txt"
tissue_tissue_correlation_file=${preprocessed_tgfm_data_dir}${gene_type}"_average_tissue_tissue_correlation_across_genes.txt"
if false; then
sbatch compute_average_tissue_tissue_correlation_in_predicted_expression_across_genes.sh $tgfm_input_summary_file $gtex_pseudotissue_file $tissue_tissue_correlation_file
fi
...
#################################
if false; then
sh run_pops_enrichment_analysis.sh $tgfm_results_dir $gene_type $ukbb_sumstats_hg38_dir"ukbb_hg38_sumstat_files_with_samp_size_and_h2_readable3.txt" $gtex_susie_gene_models_dir $preprocessed_tgfm_data_dir $pops_results_summary_file $pops_enrichment_dir $tgfm_organized_results_dir
fi
gene_type="all_non_zero_gene"
if false; then
sbatch run_pops_enrichment_analysis.sh $tgfm_results_dir $gene_type $ukbb_sumstats_hg38_dir"ukbb_hg38_sumstat_files_with_samp_size_and_h2_readable3.txt" $gtex_susie_gene_models_dir $preprocessed_tgfm_data_dir $pops_results_summary_file $pops_enrichment_dir $tgfm_organized_results_dir
fi
...
bulk_pbmc_file=${sc_pseudobulk_expression_dir}"bulk_PBMC_data_set_summary.txt"
...
if false; then
for job_number in $(seq 0 $(($num_jobs-1))); do 
	sbatch preprocess_data_for_sc_tgfm.sh $ukkbb_window_summary_file $gtex_pseudotissue_file $gtex_susie_gene_models_dir $preprocessed_sc_tgfm_data_dir $job_number $num_jobs $gene_type $pb_cell_type_file $sc_pbmc_susie_gene_models_dir
done
fi
###########
#PBMC
...
if false; then
for job_number in $(seq 0 $(($num_jobs-1))); do 
	sbatch preprocess_data_for_sc_tgfm.sh $ukkbb_window_summary_file $gtex_pseudotissue_file $gtex_susie_gene_models_dir $preprocessed_sc_pb_tgfm_data_dir $job_number $num_jobs $gene_type $bulk_pbmc_file $sc_pbmc_susie_gene_models_dir
done
fi
