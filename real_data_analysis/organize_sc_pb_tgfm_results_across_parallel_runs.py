...
def extract_pseudotissue_names(gtex_pseudotissue_file,ignore_testis=False, ignore_wb=True):
	...
	for line in f:
		...
		if data[0] == 'Testis' and ignore_testis:
			continue
		if data[0] == 'Whole_Blood' and ignore_wb:
			continue
		arr.append(data[0])
	f.close()
	return np.asarray(arr)
...
def create_mapping_from_tissue_category_to_tissue_names(gtex_pseudotissue_category_file,ignore_testis=False, ignore_wb=True):
	category_names = []
	mapping = {}
	mapping2 = {}
	mapping3 = {}
	...
	for line in f:
		...
		if tissue_name == 'Testis' and ignore_testis:
			continue
		if tissue_name == 'Whole_Blood' and ignore_wb:
			continue
...
def comparison_with_non_held_out_tissue_analysis(per_gene_pip_summary_file, per_gene_tissue_full_pip_summary_file, trait_name, held_out_tissue, old_summary_file, non_held_out_tissue_comparison_file, processed_tgfm_input_stem, ld_dir, data_file_stem):
	...
	t.close()
	return
def generate_ablated_hit_summary_file(trait_names, trait_summary_files, held_out_tissue,replication_tissue, non_ablated_hit_summary_file):
	...
	for ii, trait_name in enumerate(trait_names):
		...
		for line in f:
			...
			if held_out_tissue == 'Whole_Blood' and new_best_gt_pair_tissue == replication_tissue:
				best_proxy_tissue = 'replication_tissue'
...
def generate_held_out_tissue_summary_file_across_traits(trait_names, pip_thresh, held_out_tissue_summary_file,trait_summary_files, held_out_tissue, replication_tissue):
	...
	for trait_iter, trait_name in enumerate(trait_names):
		...
		for line in f:
			...
			new_replication_gt_pair_booler = False
			...
			if new_gt_pair_pip > pip_thresh:
				...
				elif original_tissue == 'Whole_Blood' and new_tissue == replication_tissue:
					new_replication_gt_pair_booler = True
				else:
					new_non_tagging_gt_pair_booler = True
			# Other region gene
			new_other_region_gene = False
			if new_tagging_gt_pair_booler == False and new_non_tagging_gt_pair_booler == False and new_replication_gt_pair_booler == False:
			...
			if new_replication_gt_pair_booler == True:
				gt_string = 'same gene, replication tissue'
...
tgfm_gtex_organized_results_dir = sys.argv[11]
...
tissue_names = extract_pseudotissue_names(gtex_pseudotissue_file, ignore_testis=True, ignore_wb=True)
...
tissue_categories, tissue_category_to_tissue_names, tissue_name_to_broad_category, tissue_name_to_alt_tissue, alt_tissues = create_mapping_from_tissue_category_to_tissue_names(gtex_pseudotissue_category_file, ignore_testis=True, ignore_wb=True)
...
tissue_name_to_broad_category['PBMC'] = 'sc_blood'
...
held_out_tissue = 'Whole_Blood'
replication_tissue = 'PBMC'
trait_summary_files = []
...
for trait_name in trait_names:
	...
	for model_version in model_versions:
		...
		###################################################
		data_file_stem = tgfm_results_dir + 'tgfm_results_' + trait_name + '_' + gene_type + '_ignore_Whole_Blood' + '_' + model_version 
		file_stem = tgfm_organized_results_dir + 'tgfm_results_' + trait_name + '_' + gene_type + '_ignore_Whole_Blood' + '_' + model_version
		...
		old_summary_file = tgfm_gtex_organized_results_dir + 'tgfm_results_' + trait_name + '_' + gene_type + '_' + model_version + '_tgfm_gene_tissue_hit_summary.txt'
...
for pip_thresh in pip_thresholds:
	...
	generate_held_out_tissue_summary_file_across_traits(trait_names, pip_thresh, held_out_tissue_summary_file,trait_summary_files,held_out_tissue, replication_tissue)
	#print(held_out_tissue_summary_file)
