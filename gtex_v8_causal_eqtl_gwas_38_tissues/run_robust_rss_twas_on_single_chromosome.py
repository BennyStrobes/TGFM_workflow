...
# open output file
output_file = output_dir + trait_name + '_' + chrom_num + '_summary_tgfm_robust_results_' + gene_version + '_const_1e-5_prior.txt'
...
for line in f:
	...
	if pickled_data_file == 'NA':
		...
		twas_obj = tgfm_robust_causal_twas.TGFM_CAUSAL_TWAS(estimate_prior_variance=True, prior_variance=1.0, pleiotropic_prior_variance=1.0, convergence_thresh=1e-6, max_iter=50)
		twas_obj.fit(twas_data_obj=twas_data)

		twas_fusion_obj = tgfm_robust_causal_twas.TGFM_CAUSAL_TWAS(estimate_prior_variance=True,fusion_weights=True, prior_variance=1.0, pleiotropic_prior_variance=1.0, convergence_thresh=1e-6, max_iter=50)
		...
		tgfm_twas_pkl_file = output_dir + trait_name + '_' + component_name + '_' + gene_version + '_tgfm_robust_twas_results.pkl'
		g = open(tgfm_twas_pkl_file, "wb")
		pickle.dump(twas_obj, g)
		g.close()

		# Save FUSION ROBUST TWAS results to output file
		tgfm_fusion_twas_pkl_file = output_dir + trait_name + '_' + component_name + '_' + gene_version + '_tgfm_robust_fusion_twas_results.pkl'
