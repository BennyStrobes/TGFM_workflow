...
def create_alpha_mu_and_alpha_var_objects(twas_pickle_file_names, tissue_to_position_mapping, gene_count_method, init_version):
	...
	return alpha_mu_object, alpha_var_object, tissue_object, valid_gene_object, np.asarray(twas_obj_file_names)
...
def infer_rss_twas_tissue_specific_priors(ordered_tissue_names, twas_pickle_file_names, temp_output_file, gene_count_method, init_version, fusion_weights, max_iter=600):
	# Number of tissues
...
temp_output_file = pseudotissue_gtex_rss_multivariate_twas_dir + trait_name + '_' + gene_version + '_' + gene_count_method + '_' + init_version + '_fusion_weights_' + fusion_weights + '_tissue_specific_prior_precision_temp.txt'
expected_gamma_alpha, gamma_alpha_a, gamma_alpha_b = infer_rss_twas_tissue_specific_priors(ordered_tissue_names, twas_pickle_file_names, temp_output_file, gene_count_method, init_version, fusion_weights)


output_file = pseudotissue_gtex_rss_multivariate_twas_dir + trait_name + '_' + gene_version + '_' + gene_count_method + '_' + init_version + '_fusion_weights_' + fusion_weights + '_tissue_specific_prior_precision.txt'
