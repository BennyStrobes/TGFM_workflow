...
ordered_tissue_names = extract_tissue_names(gtex_pseudotissue_file)
# open output file
output_file = output_dir + trait_name + '_' + chrom_num + '_summary_tgfm_robust_results_' + gene_version + '_fusion_' + fusion_weights + '_tissue_specific_prior.txt'
...
for line in f:
	...
	if pickled_data_file == 'NA':
		...
		if fusion_weights == 'False':
			twas_obj = tgfm_robust_causal_twas.TGFM_CAUSAL_TWAS(estimate_prior_variance=False, prior_variance_vector=ordered_gene_prior_variances, pleiotropic_prior_variance=pleiotropic_variance, convergence_thresh=1e-8)
		else:
			twas_obj = tgfm_robust_causal_twas.TGFM_CAUSAL_TWAS(fusion_weights=True, estimate_prior_variance=False, prior_variance_vector=ordered_gene_prior_variances, pleiotropic_prior_variance=pleiotropic_variance, convergence_thresh=1e-8)
		...
		tgfm_twas_pkl_file = output_dir + trait_name + '_' + component_name + '_' + gene_version + '_fusion_' + fusion_weights + '_tgfm_robust_twas_tissue_specific_prior_results.pkl'
		...
		# TGFM RSS Regression
		tgfm_rss_regression_fm_results_table_file = output_dir + trait_name + '_' + component_name + '_' + gene_version + '_fusion_' + fusion_weights + '_tgfm_robust_rss_regression_fine_mapping_tissue_specific_prior_table.txt'
		tgfm_rss_fm_obj_regr.posterior_prob_df.to_csv(tgfm_rss_regression_fm_results_table_file, sep='\t', index=False)
		tgfm_rss_regression_tissue_fm_results_table_file = output_dir + trait_name + '_' + component_name + '_' + gene_version + '_fusion_' + fusion_weights + '_tgfm_robust_rss_regression_tissue_fine_mapping_tissue_specific_prior_table.txt'
