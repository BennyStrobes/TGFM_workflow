...
import tgfm_robust_mog_causal_twas
...
# open output file
output_file = output_dir + trait_name + '_' + chrom_num + '_summary_tgfm_robust_mog_results_' + gene_version + '_const_1e-5_prior.txt'
...
for line in f:
	...
	if pickled_data_file == 'NA':
		...
		twas_obj = tgfm_robust_mog_causal_twas.TGFM_CAUSAL_TWAS(estimate_prior_distribution=True, pleiotropic_prior_variance=1.0, convergence_thresh=1e-6, max_iter=25)
		twas_obj.fit(twas_data_obj=twas_data)

		#twas_fusion_obj = tgfm_robust_causal_twas.TGFM_CAUSAL_TWAS(estimate_prior_variance=True,fusion_weights=False, prior_variance=1.0, pleiotropic_prior_variance=1.0, convergence_thresh=1e-6, max_iter=50)
		...
		tgfm_twas_pkl_file = output_dir + trait_name + '_' + component_name + '_' + gene_version + '_tgfm_robust_mog_twas_results.pkl'
		g = open(tgfm_twas_pkl_file, "wb")
		pickle.dump(twas_obj, g)
		g.close()

		# Save FUSION ROBUST TWAS results to output file
		#tgfm_fusion_twas_pkl_file = output_dir + trait_name + '_' + component_name + '_' + gene_version + '_tgfm_robust_mog_fusion_twas_results.pkl'
		...
		tgfm_rss_regression_fm_results_table_file = output_dir + trait_name + '_' + component_name + '_' + gene_version + '_tgfm_robust_mog_rss_regression_fine_mapping_table.txt'
		tgfm_rss_fm_obj_regr.posterior_prob_df.to_csv(tgfm_rss_regression_fm_results_table_file, sep='\t', index=False)
		tgfm_rss_regression_tissue_fm_results_table_file = output_dir + trait_name + '_' + component_name + '_' + gene_version + '_tgfm_robust_mog_rss_regression_tissue_fine_mapping_table.txt'
