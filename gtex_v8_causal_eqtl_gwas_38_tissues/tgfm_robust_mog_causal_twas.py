...
class TGFM_CAUSAL_TWAS(object):
	def __init__(self, max_iter=200, convergence_thresh=1e-5, estimate_prior_distribution=False, fusion_weights=False, grid_start=1e-10, grid_end=1.0, multiplicative_grid_scaling_factor=3.0, pleiotropic_prior_variance=1000, ard_a_prior=1e-16, ard_b_prior=1e-16, dirichlet_prior=1e-15):
		# Prior on gamma distributions defining residual variance and
		self.max_iter = max_iter
		self.convergence_thresh = convergence_thresh
		self.estimate_prior_distribution = estimate_prior_distribution
		self.grid_start = grid_start
		self.grid_end = grid_end
		self.multiplicative_grid_scaling_factor = multiplicative_grid_scaling_factor
	...
	def fit(self, twas_data_obj):
		...
		print('###############################')
		print('TGFM ROBUST MOG CAUSAL TWAS')
		print('###############################')
		...
		for itera in range(self.max_iter):
			# Compute expectation of log(pi)
			expected_ln_pi = scipy.special.psi(self.delta_alpha) - scipy.special.psi(np.sum(self.delta_alpha))
			expected_ln_pi_all_rows = np.repeat(np.reshape(expected_ln_pi, ((1,self.M))), self.G,axis=0)

			# Update alpha (ie predicted effect of each gene on the trait)
			self.update_alpha(expected_ln_pi_all_rows)
			# Update beta (ie predicted pleiotropic gwas effect sizes)
			self.update_beta(self.expected_gamma_beta)

			if self.estimate_prior_distribution:
				self.update_delta_alpha()
	...
	def update_alpha(self, expected_ln_pi):
		...
		for g_index in range(self.G):
			...
			a_terms = self.precomputed_a_terms[g_index] - (.5/self.variance_grid_arr)

			# VI Updates (for mixtures)
			self.mixture_alpha_var[g_index,:] = -1.0/(2.0*a_terms)
			self.mixture_alpha_mu[g_index,:] = b_term*self.mixture_alpha_var[g_index,:]

			# VI Updates (for latent variables)
			un_normalized_lv_weights = expected_ln_pi[g_index,:] - (.5*np.log(self.variance_grid_arr)) + (.5*np.square(self.mixture_alpha_mu[g_index,:])/self.mixture_alpha_var[g_index,:]) + (.5*np.log(self.mixture_alpha_var[g_index,:]))
			# log sum exp trick
			un_normalized_lv_weights = un_normalized_lv_weights - scipy.special.logsumexp(un_normalized_lv_weights)
			self.Z[g_index,:] = np.exp(un_normalized_lv_weights)

			# VI Updates (joint updates)
			self.alpha_mu[g_index] = np.sum(self.mixture_alpha_mu[g_index,:]*self.Z[g_index,:])
			self.alpha_var[g_index] = np.sum(self.mixture_alpha_var[g_index,:]*self.Z[g_index,:]) + np.sum(np.square(self.mixture_alpha_mu[g_index,:])*self.Z[g_index,:]) - np.sum(np.square(self.mixture_alpha_mu[g_index,:]*self.Z[g_index,:]))
	...
	def update_delta_alpha(self):
		self.delta_alpha = np.sum(self.Z,axis=0) + self.dirichlet_prior
	...
	def initialize_variables(self, twas_data_obj):
		...
		self.variance_grid_arr = []
		grid_value = self.grid_start
		grid_converged = False
		while grid_converged == False:
			self.variance_grid_arr.append(grid_value)
			new_grid_value = grid_value*self.multiplicative_grid_scaling_factor
			if new_grid_value > self.grid_end:
				grid_converged = True 
			else:
				grid_value = new_grid_value
		self.variance_grid_arr = np.asarray(self.variance_grid_arr)
		self.M = len(self.variance_grid_arr)

		# Initialize prior for pleiotropic effects
		self.expected_gamma_beta = 1.0/self.pleiotropic_prior_variance

		# Initialize deltas
		self.delta_alpha = self.G*np.ones(self.M)/self.M

		# Initialize latent variables for each gene
		self.Z = np.ones((self.G, self.M))/self.M
		...
		self.mixture_alpha_mu = np.zeros((self.G, self.M))
		self.mixture_alpha_var = np.ones((self.G, self.M))
		# Convert to mean and variance defining each sample
		self.alpha_mu = np.sum(self.mixture_alpha_mu*self.Z,axis=1)
		self.alpha_var = np.sum(self.mixture_alpha_var*self.Z,axis=1) + np.sum(np.square(self.mixture_alpha_mu)*self.Z,axis=1) - np.sum(np.square(self.mixture_alpha_mu*self.Z),axis=1)
