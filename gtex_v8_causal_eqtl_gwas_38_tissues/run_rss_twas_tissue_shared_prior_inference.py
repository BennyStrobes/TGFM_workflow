...
def extract_twas_pickle_file_names(trait_name, pseudotissue_gtex_rss_multivariate_twas_dir, gene_version, fusion_weights):
	...
	for chrom_num in range(1,23):
		file_name = pseudotissue_gtex_rss_multivariate_twas_dir + trait_name + '_' + str(chrom_num) + '_summary_tgfm_results_' + gene_version + '_const_1e-5_prior.txt'
		f = open(file_name)
		head_count = 0
		for line in f:
			line = line.rstrip()
...
def create_alpha_mu_and_alpha_var_objects(twas_pickle_file_names, gene_count_method, init_version):
	...
	return alpha_mu_object, alpha_var_object, valid_gene_object, np.asarray(twas_obj_file_names)
...
def update_alphas(alpha_mu_object, alpha_var_object, twas_obj_file_names, expected_gamma_alpha):
	# Number of components to loop over
	num_components = len(alpha_mu_object)
	...
	for nn in range(num_components):
		...
		#print(str(nn) + '\t' + str(twas_obj.G))
		#  create vector of length number of genes where element is tissue-specific prior precision corresponding to that gene
...
def infer_rss_twas_tissue_shared_priors(twas_pickle_file_names, temp_output_file, gene_count_method, init_version, fusion_weights, max_iter=600):
	# First create initial alpha mu object
	alpha_mu_object, alpha_var_object, valid_gene_object, twas_obj_file_names = create_alpha_mu_and_alpha_var_objects(twas_pickle_file_names, gene_count_method, init_version)
	...
	for itera in range(max_iter):
		...
		alpha_mu_object, alpha_var_object = update_alphas(alpha_mu_object, alpha_var_object, twas_obj_file_names, expected_gamma_alpha)
		# Update gamma
		gamma_alpha_a, gamma_alpha_b = update_gamma_alpha(alpha_mu_object, alpha_var_object, valid_gene_object, 1e-16, 1e-16)
...
temp_output_file = pseudotissue_gtex_rss_multivariate_twas_dir + trait_name + '_' + gene_version + '_' + gene_count_method + '_' + init_version + '_fusion_weights_' + fusion_weights + '_tissue_shared_prior_precision_temp.txt'
expected_gamma_alpha, gamma_alpha_a, gamma_alpha_b = infer_rss_twas_tissue_shared_priors(twas_pickle_file_names, temp_output_file, gene_count_method, init_version, fusion_weights)


output_file = pseudotissue_gtex_rss_multivariate_twas_dir + trait_name + '_' + gene_version + '_' + gene_count_method + '_' + init_version + '_fusion_weights_' + fusion_weights + '_tissue_shared_prior_precision.txt'
gamma_alpha_output_mat = np.vstack((np.asarray(['expected_precision', 'gamma_alpha_a', 'gamma_alpha_b']), np.asarray([gamma_alpha_a/gamma_alpha_b, gamma_alpha_a, gamma_alpha_b]).astype(str)))
