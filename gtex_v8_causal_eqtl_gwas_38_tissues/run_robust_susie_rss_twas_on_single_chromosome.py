...
# open output file
output_file = output_dir + trait_name + '_' + chrom_num + '_summary_tgfm_robust_susie_results_' + gene_version + '_const_1e-5_prior.txt'

print(output_file)
t = open(output_file,'w')
# write header
t.write('component_name\tnum_genes\ttgfm_twas_pickle\ttgfm_fusion_twas_pickle\tpickled_data_file\n')
...
for line in f:
	...
	if pickled_data_file == 'NA':
		t.write(component_name + '\t' + str(num_genes) + '\t' + 'NA' + '\t' 'NA' + '\t' + 'NA' + '\n')
	else:
		...
		twas_obj = tgfm_robust_susie_causal_twas.TGFM_CAUSAL_TWAS(estimate_prior_variance=True,fusion_weights=False, robust=True, pleiotropic_prior_variance=1e-8, convergence_thresh=1e-7, max_iter=50)
		twas_obj.fit(twas_data_obj=twas_data)

		twas_fusion_obj = tgfm_robust_susie_causal_twas.TGFM_CAUSAL_TWAS(estimate_prior_variance=True,fusion_weights=True, robust=True, pleiotropic_prior_variance=1e-8, convergence_thresh=1e-7, max_iter=50)
		...
		t.write(component_name + '\t' + str(num_genes) + '\t' + tgfm_twas_pkl_file + '\t' + tgfm_fusion_twas_pkl_file + '\t' + pickled_data_file + '\n')
f.close()
t.close()