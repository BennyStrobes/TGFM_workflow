...
create_gene_tissue_df <- function(tissue_names, pseudotissue_gtex_fusion_weights_dir, pseudotissue_gtex_susie_pmces_fusion_weights_dir, global_chrom_num, trait_name, gene_version) {
	gene_arr <- c()
	tissue_arr <- c()
	wgt_file_arr <- c()
	fusion_wgt_file_arr <- c()
	fusion_snp_file_arr <- c()
	chrom_num_arr <- c()
	tss_arr <- c()
	top_model_arr <- c()
	...
	for (tissue_iter in 1:length(tissue_names)) {
		...
		fusion_tissue_pos_file <- paste0(pseudotissue_gtex_fusion_weights_dir, tissue_name, "_", gene_version, ".pos")
		fusion_tissue_pos_df <- read.table(fusion_tissue_pos_file, header=TRUE)
		fusion_tissue_pos_df <- fusion_tissue_pos_df[fusion_tissue_pos_df$CHR==global_chrom_num,]  # Filter to genes on this chromosome


		fusion_tissue_hash <- hash()
		ngenes <- dim(fusion_tissue_pos_df)[1]
		for (gene_num in 1:ngenes) {
			gene_name <- fusion_tissue_pos_df$ID[gene_num]
			wgt_file <- fusion_tissue_pos_df$WGT[gene_num]
			snp_file <- fusion_tissue_pos_df$SNP[gene_num]
			fusion_tissue_hash[gene_name] = c(wgt_file, snp_file)
		}

		# Loop through genes
		ngenes <- dim(tissue_pos_df)[1]
		for (gene_num in 1:ngenes) {
			gene_name <- tissue_pos_df$ID[gene_num]

			if (has.key(gene_name, fusion_tissue_hash)) {

				fusion_tissue_hash_arr <- fusion_tissue_hash[[gene_name]]
				...
				fusion_wgt_file_arr <- c(fusion_wgt_file_arr, fusion_tissue_hash_arr[1])
				fusion_snp_file_arr <- c(fusion_snp_file_arr, fusion_tissue_hash_arr[2])
	...
	df <- data.frame(gene=gene_arr, tissue=tissue_arr, wgt_file=wgt_file_arr, chrom_num=chrom_num_arr, gene_tss=tss_arr, top_model=top_model_arr, fusion_wgt_file=fusion_wgt_file_arr, fusion_snp_file=fusion_snp_file_arr)
	return(df)
}
...
pseudotissue_gtex_fusion_weights_dir = args[5]
pseudotissue_gtex_susie_pmces_fusion_weights_dir=args[6]
gtex_fusion_processed_intermediate_data=args[7]
gwas_sample_size=as.numeric(args[8])
pseudotissue_gtex_rss_multivariate_twas_data_dir=args[9] # OUTPUTDIR
gene_version=args[10]
...
fusion_gene_tissue_df <- create_gene_tissue_df(tissue_names, pseudotissue_gtex_fusion_weights_dir, pseudotissue_gtex_susie_pmces_fusion_weights_dir, global_chrom_num, trait_name, gene_version)
...
for (trait_component_iter in 1:num_susie_trait_components) {
	...
	for (gene_tissue_iter in 1:num_gene_tissue_pairs) {
		...
		fusion_wgt_file <- trait_component_fusion_gene_tissue_df$fusion_wgt_file[gene_tissue_iter]
		fusion_snp_file <- trait_component_fusion_gene_tissue_df$fusion_snp_file[gene_tissue_iter]

		###########################
		# CP AND PASTE FUSION_assoc_test.R code to reconstruct LD Matrix
		###########################
		# Load fusion weights
		load(fusion_wgt_file)
		fusion_weights <- dataset$meta_weights

		# Load in fusion snp file
		load(fusion_snp_file)
		fusion_snps <- dataset
		...
		if (sum(snps$V5 != fusion_snps$V5) != 0) {
			print("SNP ASSUMPTION ERROR 1")
		}
		if (sum(snps$V2 != fusion_snps$V2) != 0) {
			print("SNP ASSUMPTION ERROR 2")
		}
		...
		fusion_weights = fusion_weights[m.keep]
		...
		fusion_weights[qc$flip] = -1*fusion_weights[qc$flip]
		...
		# Save FUSION WEIGHTS file

		fusion_weights_file_name_output <- paste0(pseudotissue_gtex_rss_multivariate_twas_data_dir, component_name,"_", gene_version,"_", gene_id, "_", tissue_id, "_fusion_weights.tsv")
		write.table(fusion_weights, fusion_weights_file_name_output, quote=FALSE, sep="\t",row.names = FALSE, col.names=FALSE)
		fusion_weights_file_names <- c(fusion_weights_file_names, fusion_weights_file_name_output)
	...
	trait_component_fusion_gene_tissue_df$fusion_weight_file_name = as.character(fusion_weights_file_names)
