...
def extract_twas_pickle_file_names(trait_name, pseudotissue_gtex_rss_multivariate_twas_dir, gene_version, fusion_weights):
	...
	for chrom_num in range(1,23):
		file_name = pseudotissue_gtex_rss_multivariate_twas_dir + trait_name + '_' + str(chrom_num) + '_summary_tgfm_robust_susie_results_' + gene_version + '_const_1e-5_prior.txt'
		f = open(file_name)
		head_count = 0
		for line in f:
			line = line.rstrip()
...
def create_alpha_mu_and_alpha_var_objects(twas_pickle_file_names, tissue_to_position_mapping, gene_count_method, init_version):
	...
	temp_file_name = twas_pickle_file_names[0]
	f = open(temp_file_name, "rb")
	twas_obj = pickle.load(f)
	f.close()
	L = twas_obj.phi.shape[0]
	...
	for itera, twas_pickle_file_name in enumerate(twas_pickle_file_names):
		...
		if init_version == 'trained_init':
			...
			phi_object.append(np.copy(twas_obj.phi))
			component_variance_obj.append(np.ones(L))
			...
			alpha_mu_object.append(np.zeros(twas_obj.alpha_mu.shape))
			alpha_var_object.append(np.ones(twas_obj.alpha_var.shape))
			phi_object.append(np.ones(twas_obj.phi.shape)/(twas_obj.phi.shape[1]))
			component_variance_obj.append(np.ones(L))
	...
	return alpha_mu_object, alpha_var_object, phi_object, component_variance_obj, beta_mu_object, beta_var_object, tissue_object, valid_gene_object, np.asarray(twas_obj_file_names)
...
def update_alphas_and_betas(alpha_mu_object, alpha_var_object, phi_object, component_variance_obj, beta_mu_object, beta_var_object, tissue_object, twas_obj_file_names, tissue_variances, expected_gamma_beta):
	# Number of components to loop over
	num_components = len(alpha_mu_object)
	...
	for nn in range(num_components):
		...
		component_variances = np.zeros(len(tissue_object[nn]))
		for pos in range(len(tissue_object[nn])):
			component_variances[pos] = tissue_variances[tissue_object[nn][pos]]
		component_pi = component_variances/np.sum(component_variances)
		...
		'''
		temp_gene_pred = np.dot(np.sum(twas_obj.alpha_mu*twas_obj.phi,axis=0), twas_obj.gene_eqtl_pmces) - np.dot(np.sum(alpha_mu_object[nn]*phi_object[nn],axis=0), twas_obj.gene_eqtl_pmces)
		...
		twas_obj.phi = np.copy(phi_object[nn])
		...
		twas_obj.component_variances = np.copy(component_variance_obj[nn])

		# Perform VI UPDATES on alpha_mu and alpha_var
		twas_obj.update_susie_alpha(component_pi, twas_obj.component_variances)
		...
		# Perform VI Updates on component variances
		twas_obj.update_component_variances()
		component_variance_obj[nn] = twas_obj.component_variances

	return alpha_mu_object, alpha_var_object, beta_mu_object, beta_var_object

def infer_rss_twas_tissue_specific_priors(ordered_tissue_names, twas_pickle_file_names, temp_output_file, temp_output_file2, gene_count_method,tissue_variances, init_version, max_iter=600):
	# Number of tissues
	...
	alpha_mu_object, alpha_var_object, phi_object, component_variance_obj, beta_mu_object, beta_var_object, tissue_object, valid_gene_object, twas_obj_file_names = create_alpha_mu_and_alpha_var_objects(twas_pickle_file_names, tissue_to_position_mapping, gene_count_method, init_version)
	...
	for itera in range(max_iter):
		...
		alpha_mu_object, alpha_var_object, beta_mu_object, beta_var_object = update_alphas_and_betas(alpha_mu_object, alpha_var_object, phi_object, component_variance_obj, beta_mu_object, beta_var_object, tissue_object, twas_obj_file_names, tissue_variances, expected_gamma_beta)
...
tissue_precision_file = pseudotissue_gtex_rss_multivariate_twas_dir + trait_name + '_' + gene_version + '_' + gene_count_method + '_' + init_version + '_fusion_weights_' + fusion_weights  + '_robust_tissue_specific_prior_precision_temp.txt'
tissue_variances = 1.0/np.loadtxt(tissue_precision_file,dtype=str,delimiter='\t')[1:,1].astype(float)

temp_output_file = pseudotissue_gtex_rss_multivariate_twas_dir + trait_name + '_' + gene_version + '_' + gene_count_method + '_' + init_version + '_fusion_weights_' + fusion_weights  + '_robust_susie_tissue_specific_prior_precision_temp.txt'
temp_output_file2 = pseudotissue_gtex_rss_multivariate_twas_dir + trait_name + '_' + gene_version + '_' + gene_count_method + '_' + init_version + '_fusion_weights_' + fusion_weights  + '_robust_susie_pleiotropic_prior_precision_temp.txt'


gamma_beta_a, gamma_beta_b = infer_rss_twas_tissue_specific_priors(ordered_tissue_names, twas_pickle_file_names, temp_output_file, temp_output_file2, gene_count_method, tissue_variances, init_version)


#output_file = pseudotissue_gtex_rss_multivariate_twas_dir + trait_name + '_' + gene_version + '_' + gene_count_method + '_' + init_version + '_fusion_weights_' + fusion_weights  + '_robust_susie_tissue_specific_prior_precision.txt'
...
output_file2 = pseudotissue_gtex_rss_multivariate_twas_dir + trait_name + '_' + gene_version + '_' + gene_count_method + '_' + init_version + '_fusion_weights_' + fusion_weights  + '_robust_susie_pleiotropic_prior_precision.txt'
gamma_beta_output_mat = np.vstack((np.asarray(['expected_precision', 'gamma_beta_a', 'gamma_beta_b']), np.asarray([gamma_beta_a/gamma_beta_b, gamma_beta_a, gamma_beta_b]).astype(str)))
