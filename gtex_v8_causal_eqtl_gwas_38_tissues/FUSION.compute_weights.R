...
weights.bslmm = function( input , bv_type , snp , out=NA ) {
	...
	# ADDED BY ME (ISSUE WITH UPDATED GEMMA)
	char_arr = strsplit(out, "/")[[1]]
	char_arr = char_arr[2:length(char_arr)]
	out_dir = paste0("/", paste(char_arr[1:(length(char_arr)-1)],collapse="/"), "/")
	out_file = char_arr[length(char_arr)]
	arg = paste( opt$PATH_gemma , " -miss 1 -maf 0 -r2 1 -rpace 1000 -wpace 1000 -bfile " , input , " -bslmm " , bv_type , " -outdir ", out_dir, " -o " , out_file , sep='' )
...
arg = paste( opt$PATH_plink ," --allow-no-sex --bfile ",opt$bfile," --pheno ",pheno.file," --keep ",pheno.file," --make-bed --keep-allele-order --out ",geno.file,sep='')
system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
...
############################
# ADDED BY BEN STROBER
#############################
# THROW OUT ANY AMBIGOUS SNPS (no need. dealth with earlier)
#a1 = genos$bim$V5
#a2 = genos$bim$V6
...
for ( i in 1:opt$crossval ) {
	...
	arg = paste( opt$PATH_plink ," --allow-no-sex --bfile ",opt$tmp," --keep ",cv.file,".keep --out ",cv.file," --make-bed --keep-allele-order",sep='')
	system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
