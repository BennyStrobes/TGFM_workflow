...
def get_gtex_tissues(gtex_tissue_file):
	...
	pseudotissue_to_sample_size_dictionary = {}
	head_count = 0
	arr = []
	arr2 = []
	for line in f:
		...
		arr.append(data[0])
		arr2.append(data[2])
		pseudotissue_to_sample_size_dictionary[data[0]] = float(data[1])
	f.close()
	return np.asarray(arr), np.asarray(arr2), pseudotissue_to_sample_size_dictionary
...
def fill_in_gene_dicti_from_single_tissue_summary_stats(sum_stats_file, gene_dicti, gtex_tissue):
	...
	for line in f:
		...
		beta = float(data[2])
		t_stat = float(data[3])
		# cases where t_stat == 0.0 (very rare)
		if t_stat == 0.0:
			t_stat = 1e-13
...
gtex_tissues, meta_analyzed_booleans, pseudotissue_to_sample_size_dictionary = get_gtex_tissues(gtex_tissue_file)
...
for gene_index, test_gene in enumerate(ordered_genes):
	...
	for iii, pseudo_tissue_name in enumerate(gene_tissue_arr):
		samp_size_vec[iii] = pseudotissue_to_sample_size_dictionary[pseudo_tissue_name]
