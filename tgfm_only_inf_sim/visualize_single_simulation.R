...
make_tgfm_pip_fdr_plot_varying_n_eqtl_method_and_initialization_method <- function(df, pip_threshold) {
	df$fdr = 1.0 - df$coverage
	df$fdr_lb = 1.0 - df$coverage_ub
	df$fdr_ub = 1.0 - df$coverage_lb

	df$n_eqtl_per_gene <- factor(df$n_eqtl_per_gene, levels=c("2", "4", "6", "8", "10"))

	p<-ggplot(data=df, aes(x=n_eqtl_per_gene, y=fdr, fill=initialization_version)) +
  		geom_bar(stat="identity", position=position_dodge()) +
  		geom_errorbar(aes(ymin=fdr_lb, ymax=fdr_ub), width=.4, position=position_dodge(.9))  +
  		figure_theme() +
  		labs(x="# Simulated eQTL / gene", y="FDR", fill="", title=paste0("PIP=", pip_threshold))  +
  		geom_hline(yintercept=(1.0-as.numeric(pip_threshold)), linetype=2) +
  		theme(plot.title = element_text(hjust = 0.5,size=12))
  	return(p)	
}

make_tgfm_pip_fdr_plot_varying_n_eqtl_method_and_element_class <- function(df, pip_threshold) {
	df$fdr = 1.0 - df$coverage
	df$fdr_lb = 1.0 - df$coverage_ub
	df$fdr_ub = 1.0 - df$coverage_lb
	...
	p<-ggplot(data=df, aes(x=n_eqtl_per_gene, y=fdr, fill=genetic_element_class)) +
  		geom_bar(stat="identity", position=position_dodge()) +
  		geom_errorbar(aes(ymin=fdr_lb, ymax=fdr_ub), width=.4, position=position_dodge(.9))  +
  		figure_theme() +
  		labs(x="# Simulated eQTL / gene", y="FDR", fill="", title=paste0("PIP=", pip_threshold))  +
...
make_gene_causal_element_directional_ld_sum_fp_strat_histogram <- function(df) {
	info <- wilcox.test(df$causal_element_directional_ld_sum[df$false_positive_gene=="True"], df$causal_element_directional_ld_sum[df$false_positive_gene=="False"], alternative = "two.sided")
	pvalue = info$p.value
	pp <- ggplot(df, aes(x=causal_element_directional_ld_sum, color=false_positive_gene)) +
  		geom_histogram(aes(y = ..density..),fill="white", alpha=0.5, position="identity") +
  		figure_theme() +
  		labs(x=paste0("Causal genetic element directional ld sum / gene"), color="FP gene",title=paste0("PIP: 0.9", " / p-value=", pvalue)) 
  	return(pp)
}

make_gene_causal_element_identified_ld_score_fp_strat_histogram <- function(df) {
	df = df[df$identified_element_ld_score < 20.0,]
	info <- wilcox.test(df$identified_element_ld_score[df$false_positive_gene=="True"], df$identified_element_ld_score[df$false_positive_gene=="False"], alternative = "two.sided")
	pvalue = info$p.value
	pp <- ggplot(df, aes(x=identified_element_ld_score, color=false_positive_gene)) +
  		geom_histogram(aes(y = ..density..),fill="white", alpha=0.5, position="identity") +
  		figure_theme() +
  		labs(x=paste0("TGFM-identified element ld score / gene"), color="FP gene",title=paste0("PIP: 0.9", " / p-value=", pvalue)) 
  	return(pp)	
}

make_gene_causal_element_identified_ld_score_fp_strat_histogram2 <- function(df) {
	df = df[df$identified_element_ld_score < 20.0,]
	info <- wilcox.test(df$identified_element_ld_score[df$false_positive_gene=="True"], df$identified_element_ld_score[df$false_positive_gene=="False"], alternative = "two.sided")
	pvalue = info$p.value
	pp <- ggplot(df, aes(x=identified_element_ld_score, color=false_positive_gene)) +
  		geom_histogram(aes(y = ..density..),fill="white", alpha=0.5, position="identity") +
  		figure_theme() +
  		labs(x=paste0("LD Score (causal variant not identiied by variant TGFM) / gene"), color="FP gene",title=paste0("PIP: 0.9", " / p-value=", pvalue)) 
  	return(pp)	
}

make_causal_variant_pip_fp_strat_histogram <- function(df) {
	info <- wilcox.test(df$PIP[df$tag_false_positive_gene=="True"], df$PIP[df$tag_false_positive_gene=="False"], alternative = "greater")
	pvalue = info$p.value
	print(info)
	pp <- ggplot(df, aes(x=PIP, color=tag_false_positive_gene)) +
  		geom_histogram(aes(y = ..density..),fill="white", alpha=0.5, position="identity") +
  		figure_theme() +
  		labs(x=paste0("PIP of causal non-mediated variant"), color="Tag FP gene",title=paste0("PIP: 0.9", " / p-value=", pvalue)) 
  	return(pp)	
}
...
if (FALSE) {
#####################################################################
# Calibration as a function of genes and variants and n_eqtls
#####################################################################

# Load in data
pip_threshold="0.9"
calibration_file <- paste0(simulated_organized_results_dir, "organized_simulation_", global_simulation_name_string,"_tgfm_inf_eqtl_pip_", pip_threshold, "_calibration.txt")
...
calibration_df = calibration_df[as.character(calibration_df$initialization_version) == "best",]
calibration_barplot_9 <- make_tgfm_pip_fdr_plot_varying_n_eqtl_method_and_element_class(calibration_df, pip_threshold)
...
calibration_df = calibration_df[as.character(calibration_df$genetic_element_class) != "all",]
calibration_df = calibration_df[as.character(calibration_df$initialization_version) == "best",]
calibration_barplot_95 <- make_tgfm_pip_fdr_plot_varying_n_eqtl_method_and_element_class(calibration_df, pip_threshold)
...
calibration_df = calibration_df[as.character(calibration_df$genetic_element_class) != "all",]
calibration_df = calibration_df[as.character(calibration_df$initialization_version) == "best",]
calibration_barplot_99 <- make_tgfm_pip_fdr_plot_varying_n_eqtl_method_and_element_class(calibration_df, pip_threshold)



output_file <- paste0(visualize_simulated_results_dir, "simulation_", global_simulation_name_string, "_calibration_baplot_vary_n_eqtl_and_element_class_cross_pip.pdf")
joint_plot <- plot_grid(calibration_barplot_9, calibration_barplot_95,calibration_barplot_99, ncol=1)
ggsave(joint_plot, file=output_file, width=7.2, height=5.9, units="in")



#####################################################################
# Calibration as a function of n_eqtls and susie inf
#####################################################################
...
calibration_barplot_9 <- make_tgfm_pip_fdr_plot_varying_n_eqtl_method_and_initialization_method(calibration_df, pip_threshold)
...
calibration_df = calibration_df[as.character(calibration_df$genetic_element_class) != "all",]
calibration_df = calibration_df[as.character(calibration_df$genetic_element_class) == "gene",]
calibration_barplot_95 <- make_tgfm_pip_fdr_plot_varying_n_eqtl_method_and_initialization_method(calibration_df, pip_threshold)
...
calibration_df = calibration_df[as.character(calibration_df$genetic_element_class) != "all",]
calibration_df = calibration_df[as.character(calibration_df$genetic_element_class) == "gene",]
calibration_barplot_99 <- make_tgfm_pip_fdr_plot_varying_n_eqtl_method_and_initialization_method(calibration_df, pip_threshold)

output_file <- paste0(visualize_simulated_results_dir, "simulation_", global_simulation_name_string, "_calibration_baplot_vary_n_eqtl_and_susie_inf_gene_only_cross_pip.pdf")
joint_plot <- plot_grid(calibration_barplot_9, calibration_barplot_95,calibration_barplot_99, ncol=1)
ggsave(joint_plot, file=output_file, width=7.2, height=5.9, units="in")
}
...
# General sim data
#####################################################################
# Plot causal-variant PIPs stratified by whether or not causal variant tags a false positive gene
gene_causal_element_ld_score_file = paste0(simulated_organized_results_dir, "organized_simulation_", global_simulation_name_string, "_n_eqtl_per_gene_6_tgfm_inf_susie_pmces_causal_snp_pip.txt")
gene_causal_element_ldscore_df <- read.table(gene_causal_element_ld_score_file, header=TRUE)
gene_causal_element_ld_score_fp_strat_histogram <- make_causal_variant_pip_fp_strat_histogram(gene_causal_element_ldscore_df)
output_file <- paste0(visualize_simulated_results_dir, "simulation_", global_simulation_name_string, "_causal_variant_pip_fp_strat_histogram.pdf")
...
gene_causal_element_ld_score_file = paste0(simulated_organized_results_dir, "organized_simulation_", global_simulation_name_string, "_n_eqtl_per_gene_6_tgfm_inf_susie_pmces_gene_causal_genetic_element_ld_scores.txt")
gene_causal_element_ldscore_df <- read.table(gene_causal_element_ld_score_file, header=TRUE)
...
# Plot causal-genetic element LD sum/ gene stratefied by false positive status
gene_causal_element_ld_score_file = paste0(simulated_organized_results_dir, "organized_simulation_", global_simulation_name_string, "_n_eqtl_per_gene_6_tgfm_inf_susie_pmces_gene_causal_genetic_element_directional_ld_sum.txt")
gene_causal_element_ldscore_df <- read.table(gene_causal_element_ld_score_file, header=TRUE)
gene_causal_element_ld_score_fp_strat_histogram <- make_gene_causal_element_directional_ld_sum_fp_strat_histogram(gene_causal_element_ldscore_df)
output_file <- paste0(visualize_simulated_results_dir, "simulation_", global_simulation_name_string, "_gene_causal_element_directional_ld_sum_fp_strat_histogram.pdf")
ggsave(gene_causal_element_ld_score_fp_strat_histogram, file=output_file, width=7.2, height=3.7, units="in")


# Plot identified genetic element LD sum/ gene stratefied by false positive status
gene_causal_element_ld_score_file = paste0(simulated_organized_results_dir, "organized_simulation_", global_simulation_name_string, "_n_eqtl_per_gene_6_tgfm_inf_susie_pmces_identified_gene_identified_non_mediated_variant_ld_scores.txt")
gene_causal_element_ldscore_df <- read.table(gene_causal_element_ld_score_file, header=TRUE)
gene_causal_element_ld_score_fp_strat_histogram <- make_gene_causal_element_identified_ld_score_fp_strat_histogram(gene_causal_element_ldscore_df)
output_file <- paste0(visualize_simulated_results_dir, "simulation_", global_simulation_name_string, "_gene_causal_element_identified_ld_score_fp_strat_histogram.pdf")
ggsave(gene_causal_element_ld_score_fp_strat_histogram, file=output_file, width=7.2, height=3.7, units="in")


# Plot identified genetic element LD sum/ gene stratefied by false positive status
gene_causal_element_ld_score_file = paste0(simulated_organized_results_dir, "organized_simulation_", global_simulation_name_string, "_n_eqtl_per_gene_6_tgfm_inf_susie_pmces_identified_gene_unidentified_causal_non_mediated_variant_ld_scores.txt")
gene_causal_element_ldscore_df <- read.table(gene_causal_element_ld_score_file, header=TRUE)
gene_causal_element_ld_score_fp_strat_histogram <- make_gene_causal_element_identified_ld_score_fp_strat_histogram2(gene_causal_element_ldscore_df)
output_file <- paste0(visualize_simulated_results_dir, "simulation_", global_simulation_name_string, "_gene_causal_element_unidentified_causal_variant_fp_strat_histogram.pdf")
ggsave(gene_causal_element_ld_score_fp_strat_histogram, file=output_file, width=7.2, height=3.7, units="in")
