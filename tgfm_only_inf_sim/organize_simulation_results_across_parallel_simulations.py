...
def extract_successfully_completed_simulation_runs(simulated_tgfm_results_dir, global_simulation_name_string):
	...
	for sim_iter in range(300):
		valid_sim1 = False 
		valid_sim2 = False 
		file_name5 = 'simulation_' + str(sim_iter) + '_' + global_simulation_name_string + '_n_eqtl_per_gene_10_eqtl_ss_inf_susie_pmces_ln_pi_uniform_init_best_resid_var_False_tgfm_component_cs_summary.txt'
		if os.path.isfile(simulated_tgfm_results_dir + file_name5):
...
def create_file_containing_tgfm_high_pip_snp_power_per_component(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, ln_pi_methods, simulated_trait_dir, simulated_tgfm_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, cs_power_per_component_output_file, pip_threshold, initialization_versions, twas_methods, resid_var_versions, n_eqtl_per_gene):
	...
	t.write('eQTL_sample_size\tln_pi_method\tinitialization_version\twas_method\tresidual_variance\tn_eqtl_per_gene\tsimulation_number\twindow_name\tgenetic_element_class\tgenetic_element_name\tcausal_variant_in_cs\n')
	# First loop through simulations
	for simulation_number in simulation_runs:
		for n_eqtl in n_eqtl_per_gene:
			# Extract dictionary list of causal genetic elements
			causal_variant_file = simulated_trait_dir + 'simulation_' + str(simulation_number) + '_' + global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl + '_non_mediated_variant_causal_effect_sizes.txt'
			causal_gene_file = simulated_trait_dir + 'simulation_' + str(simulation_number) + '_' + global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl + '_expression_mediated_gene_causal_effect_sizes.txt'
			...
			for eqtl_sample_size in eqtl_sample_sizes:
					...
					for initialization_version in initialization_versions:
						for twas_method in twas_methods:
							for resid_var_version in resid_var_versions:
								discovered_pi_dicti = {}
								cs_file = simulated_tgfm_results_dir + 'simulation_' + str(simulation_number) + '_' + global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl + '_eqtl_ss_' + str(eqtl_sample_size) + '_' + twas_method + '_ln_pi_' + ln_pi_method + '_init_' + initialization_version + '_resid_var_' + resid_var_version + '_tgfm_component_cs_summary.txt'
								f = open(cs_file)
								head_count = 0
								for line in f:
									line = line.rstrip()
				...
				tgfm_window_file = simulated_tgfm_input_data_dir + 'simulation_' + str(simulation_number) + '_' + global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl + '_tgfm_windows.txt'
				head_count = 0
									...
									for twas_method in twas_methods:
										for resid_var_version in resid_var_versions:
											...
											if gene_name in discovered_dicti[ln_pi_method + '_' + initialization_version + '_' + twas_method + '_' + resid_var_version]:
												booler = 1.0
											t.write(str(eqtl_sample_size) + '\t' + ln_pi_method + '\t' + initialization_version + '\t' + twas_method + '\t' + resid_var_version + '\t' + n_eqtl + '\t' + str(simulation_number) + '\t' + window_name + '\t' + 'gene\t' + gene_name + '\t' + str(booler) + '\n')
								...
								for twas_method in twas_methods:
									for resid_var_version in resid_var_versions:
										...
										if variant_name in discovered_dicti[ln_pi_method + '_' + initialization_version + '_' + twas_method + '_' + resid_var_version]:
											booler = 1.0
										t.write(str(eqtl_sample_size) + '\t' + ln_pi_method + '\t' + initialization_version + '\t' + twas_method + '\t' + resid_var_version + '\t' + n_eqtl + '\t' + str(simulation_number) + '\t' + window_name + '\t' + 'variant\t' + variant_name + '\t' + str(booler) + '\n')
			f.close()
	t.close()
	return
...
def create_file_containing_tgfm_cs_calibration_per_high_pip_snp(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, ln_pi_methods, twas_methods, simulated_trait_dir, simulated_tgfm_results_dir, pip_threshold, initialization_versions, resid_var_versions, n_eqtl_per_gene, cs_coverage_per_high_pip_snp_output_file):
	# Open output file handle
	t = open(cs_coverage_per_high_pip_snp_output_file,'w')
	t.write('eQTL_sample_size\tln_pi_method\tinitialization_method\ttwas_method\tresidual_variance\tn_eqtl_per_gene\tsimulation_number\twindow_name\tcomponent_number\tgenetic_element_name\tgenetic_element_class\tcausal_genetic_element_in_cs\n')
						...
						for twas_method in twas_methods:
							for resid_var_version in resid_var_versions:
								...
								for line in f:
									...
									t.write(str(eqtl_sample_size) + '\t' + ln_pi_method + '\t' + initialization_version + '\t' + twas_method + '\t' + resid_var_version + '\t' + n_eqtl + '\t' + str(simulation_number) + '\t' + component_window_name + '\t' + component_num + '\t' + genetic_element_name + '\t' + class_name  + '\t' + str(int(causal_genetic_element_in_cs_boolean)) + '\n')
								f.close()
	t.close()
	return
...
def create_file_containing_averaged_tgfm_cs_power(cs_power_per_component_output_file, cs_power_output_file, eqtl_sample_sizes, ln_pi_methods, initialization_versions, twas_methods, resid_var_versions, n_eqtl_per_gene):
	per_component_power = np.loadtxt(cs_power_per_component_output_file, dtype=str)[1:,:]
	t = open(cs_power_output_file,'w')
	t.write(('eQTL_sample_size\tln_pi_method\tinitialization_version\ttwas_methods\tresidual_variance\tn_eqtl_per_gene\tgenetic_element_class\tpower\tpower_lb\tpower_ub\n'))
					...
					for resid_var_version in resid_var_versions:
						for n_eqtl in n_eqtl_per_gene:
							for element_class in genetic_element_classes:
								...
								subset_indices = (per_component_power[:,0] == str(eqtl_sample_size)) & (per_component_power[:,1] == ln_pi_method) & (per_component_power[:,2] == initialization_version) & (per_component_power[:,3] == twas_method) & (per_component_power[:,4] == resid_var_version) & (per_component_power[:,5] == n_eqtl) & ( per_component_power[:,8] == element_class)
								...
								t.write(str(eqtl_sample_size) + '\t' + ln_pi_method + '\t' + initialization_version + '\t' + twas_method + '\t' + resid_var_version + '\t' + n_eqtl + '\t' + element_class + '\t' + str(prop) + '\t' + str(prop_lb) + '\t' + str(prop_ub) + '\n')
...
def create_file_containing_averaged_tgfm_high_pip_calibration(cs_coverage_per_component_output_file, cs_coverage_output_file, eqtl_sample_sizes, ln_pi_methods, initialization_versions, twas_methods, resid_var_versions, n_eqtl_per_gene):
	per_component_coverage = np.loadtxt(cs_coverage_per_component_output_file, dtype=str)[1:,:]
	t = open(cs_coverage_output_file,'w')
	t.write('eQTL_sample_size\tln_pi_method\tinitialization_version\ttwas_method\tresidual_variance\tn_eqtl_per_gene\tgenetic_element_class\tn_elements\tcoverage\tcoverage_lb\tcoverage_ub\n')
		...
		for ln_pi_method in ln_pi_methods:
			for initialization_version in initialization_versions:
				for twas_method in twas_methods:
					for resid_var_version in resid_var_versions:
						for n_eqtl in n_eqtl_per_gene:
							subset_indices = (per_component_coverage[:,0] == str(eqtl_sample_size)) & (per_component_coverage[:,1] == ln_pi_method) & (per_component_coverage[:,2] == initialization_version) & (per_component_coverage[:,3] == twas_method) & (per_component_coverage[:,4] == resid_var_version) & (per_component_coverage[:,5] == n_eqtl)
							components_covered = (per_component_coverage[subset_indices,:][:,-1]).astype(float)
							prop = np.sum(components_covered)/len(components_covered)
							prop_se = np.sqrt((prop)*(1.0-prop))/np.sqrt(len(components_covered))
							prop_lb = prop - (1.96*prop_se)
							...
							t.write(str(eqtl_sample_size) + '\t' + ln_pi_method + '\t' + initialization_version +'\t' + twas_method + '\t' + resid_var_version + '\t' + n_eqtl + '\t' + 'all' + '\t' + str(n_elements) + '\t' + str(prop) + '\t' + str(prop_lb) + '\t' + str(prop_ub) + '\n')

							subset_indices = (per_component_coverage[:,0] == str(eqtl_sample_size)) & (per_component_coverage[:,1] == ln_pi_method) & (per_component_coverage[:,10] == 'variant') & (per_component_coverage[:,2] == initialization_version) & (per_component_coverage[:,3] == twas_method) & (per_component_coverage[:,4] == resid_var_version) & (per_component_coverage[:,5] == n_eqtl)
							components_covered = (per_component_coverage[subset_indices,:][:,-1]).astype(float)
							prop = np.sum(components_covered)/len(components_covered)
							prop_se = np.sqrt((prop)*(1.0-prop))/np.sqrt(len(components_covered))
							prop_lb = prop - (1.96*prop_se)
							prop_ub = prop + (1.96*prop_se)
							n_elements = np.sum(subset_indices)
							t.write(str(eqtl_sample_size) + '\t' + ln_pi_method + '\t' + initialization_version +'\t' + twas_method + '\t' + resid_var_version + '\t' + n_eqtl + '\t' + 'variant' + '\t' + str(n_elements) + '\t' + str(prop) + '\t' + str(prop_lb) + '\t' + str(prop_ub) + '\n')

							subset_indices = (per_component_coverage[:,0] == str(eqtl_sample_size)) & (per_component_coverage[:,1] == ln_pi_method) & (per_component_coverage[:,10] == 'gene') & (per_component_coverage[:,2] == initialization_version) & (per_component_coverage[:,3] == twas_method) & (per_component_coverage[:,4] == resid_var_version) & (per_component_coverage[:,5] == n_eqtl)
							components_covered = (per_component_coverage[subset_indices,:][:,-1]).astype(float)
							prop = np.sum(components_covered)/len(components_covered)
							prop_se = np.sqrt((prop)*(1.0-prop))/np.sqrt(len(components_covered))
							prop_lb = prop - (1.96*prop_se)
							prop_ub = prop + (1.96*prop_se)
							n_elements = np.sum(subset_indices)			
							t.write(str(eqtl_sample_size) + '\t' + ln_pi_method + '\t' + initialization_version + '\t' + twas_method + '\t' + resid_var_version + '\t' + n_eqtl + '\t' + 'gene' + '\t' + str(n_elements) + '\t' + str(prop) + '\t' + str(prop_lb) + '\t' + str(prop_ub) + '\n')
	t.close()
	return
...
def extract_other_identified_variants_and_genes(cs_summary_file, component_num, window_name):
	identified_elements = {}
	f = open(cs_summary_file)
	head_count = 0
	for line in f:
		...
		if head_count == 0:
			head_count = head_count + 1
			continue
		if window_name != data[0]:
			continue
		if data[1] == str(component_num):
			continue
		genetic_elements = np.asarray(data[3].split(';'))
		genetic_element_pis = np.asarray(data[4].split(';')).astype(float)
		for ii, genetic_element in enumerate(genetic_elements):
			genetic_element_pi = genetic_element_pis[ii]
			identified_elements[genetic_element] = genetic_element_pi		
	f.close()
	return identified_elements

def compute_ld_scores_between_identified_genes_and_unidentified_causal_non_mediated_variants(simulation_runs,global_simulation_name_string_small, global_simulation_name_string, eqtl_sample_size, twas_method, ln_pi_method, simulated_trait_dir, simulated_tgfm_results_dir, simulated_gene_position_file, simulated_gene_expression_dir, simulated_tgfm_input_data_dir,ld_score_output_file, cs_coverage_per_high_pip_snp_output_file, n_eqtl):
	...
	for line in f:
		...
		if data[2] != 'best':
			continue
		n_eqtl_per_gene = data[5]
		if n_eqtl_per_gene != n_eqtl:
			continue
		sim_number = data[6]
		window_name = data[7]
		ele_class = data[-2]
		...
		if full_window_name not in fp_dicti:
			temp_dicti = {}
			temp_dicti[gene_name] = component_num
			fp_dicti[full_window_name] = temp_dicti
	f.close()


	t = open(ld_score_output_file,'w')
	t.write('simulation_number\twindow_name\tgene_name\tidentified_element_ld_score\tfalse_positive_gene\n')
	for simulation_number in simulation_runs:
		...
		for line in f:
			...
			# Need to extract unidentified (by variant model) causal genetic variants
			variant_only_tgfm_pkl = simulated_tgfm_results_dir + 'simulation_' + str(simulation_number) + '_' + global_simulation_name_string + '_eqtl_ss_' + str(eqtl_sample_size) + '_' + twas_method + '_ln_pi_uniform_init_best_resid_var_False_' + window_name + '_variant_only_results.pkl'
			g = open(variant_only_tgfm_pkl, "rb")
			variant_only_tgfm_res = pickle.load(g)
			unidentified_causal_genetic_variants = {}
			g.close()
			for var_iter, var_name in enumerate(variant_only_tgfm_res['variants']):
				if var_name not in causal_variants:
					continue
				if np.max(variant_only_tgfm_res['beta_phi'][:,var_iter]) < .01:
					unidentified_causal_genetic_variants[var_name] = 1
			...
			counter = 0
			all_identified_indices = []
			for gene in tgfm_data['genes']:
				if gene in unidentified_causal_genetic_variants:
					all_identified_indices.append(counter)
				counter = counter + 1
			for variant in tgfm_data['variants']:
				if variant in unidentified_causal_genetic_variants:
					all_identified_indices.append(counter)
				counter = counter + 1
			all_identified_indices = np.asarray(all_identified_indices)	
			if len(all_identified_indices) == 0:
				continue
			...
			for ii, middle_gene in enumerate(middle_genes):
				...
				if middle_gene in window_dicti:
					...
					fp_bool = 'False'
				ld_score = np.sum(np.square(middle_gene_vs_all_ld[ii,all_identified_indices]))
				t.write(str(simulation_number) + '\t' + window_name + '\t' + middle_gene + '\t' + str(ld_score) + '\t' + fp_bool + '\n')
		f.close()
		t.flush()
	t.close()
...
def compute_ld_scores_between_identified_genes_and_identified_non_mediated_variants(simulation_runs,global_simulation_name_string_small, global_simulation_name_string, eqtl_sample_size, twas_method, ln_pi_method, simulated_trait_dir, simulated_tgfm_results_dir, simulated_gene_position_file, simulated_gene_expression_dir, simulated_tgfm_input_data_dir,ld_score_output_file, cs_coverage_per_high_pip_snp_output_file, n_eqtl):
	...
	for simulation_number in simulation_runs:
		...
		cs_summary_file = simulated_tgfm_results_dir  + 'simulation_' + str(simulation_number) + '_' + global_simulation_name_string + '_eqtl_ss_' + str(eqtl_sample_size) + '_' + twas_method + '_ln_pi_uniform_init_best_resid_var_False_tgfm_component_cs_summary.txt'
		...
		for line in f:
			...
			cs_summary_file = simulated_tgfm_results_dir  + 'simulation_' + str(simulation_number) + '_' + global_simulation_name_string + '_eqtl_ss_' + str(eqtl_sample_size) + '_' + twas_method + '_ln_pi_uniform_init_best_resid_var_False_tgfm_component_cs_summary.txt'
			all_idntified_elements = extract_other_identified_variants_and_genes(cs_summary_file, 50, window_name)
			all_identified_indices = []
			counter = 0
			for gene in tgfm_data['genes']:
				if gene in all_idntified_elements:
					all_identified_indices.append(counter)
				counter = counter + 1
			for variant in tgfm_data['variants']:
				if variant in all_idntified_elements:
					all_identified_indices.append(counter)
				counter = counter + 1
			all_identified_indices = np.asarray(all_identified_indices)	
			if len(all_identified_indices) == 0:
			...
			for ii, middle_gene in enumerate(middle_genes):
				...
				if middle_gene in window_dicti:
					fp_bool = 'True'
					component_num = window_dicti[middle_gene]
					cs_summary_file = simulated_tgfm_results_dir  + 'simulation_' + str(simulation_number) + '_' + global_simulation_name_string + '_eqtl_ss_' + str(eqtl_sample_size) + '_' + twas_method + '_ln_pi_uniform_init_best_resid_var_False_tgfm_component_cs_summary.txt'
					idntified_elements = extract_other_identified_variants_and_genes(cs_summary_file, component_num, window_name)
					identified_indices = []
					counter = 0
					for gene in tgfm_data['genes']:
						if gene in idntified_elements:
							identified_indices.append(counter)
						counter = counter + 1
					for variant in tgfm_data['variants']:
						if variant in idntified_elements:
							identified_indices.append(counter)
						counter = counter + 1
					identified_indices = np.asarray(identified_indices)	
					if len(identified_indices) == 0:
						continue
					ld_score = np.sum(np.square(middle_gene_vs_all_ld[ii,identified_indices]))
					t.write(str(simulation_number) + '\t' + window_name + '\t' + middle_gene + '\t' + str(ld_score) + '\t' + fp_bool + '\n')
				else:
					fp_bool = 'False'
					ld_score = np.sum(np.square(middle_gene_vs_all_ld[ii,all_identified_indices]))
...
def compute_max_chi_squared_per_window(simulation_runs,global_simulation_name_string, eqtl_sample_size, twas_method, ln_pi_method, simulated_trait_dir, simulated_tgfm_results_dir, simulated_gene_position_file, simulated_gene_expression_dir, simulated_tgfm_input_data_dir,ld_score_output_file, cs_coverage_per_high_pip_snp_output_file, n_eqtl):
	...
	for line in f:
		...
		if fp_status == '0':
			fp_dicti[full_window_name] = 1
		elif fp_status == '1':
			tp_dicti[full_window_name] = 1
	f.close()
	t = open(ld_score_output_file,'w')
	t.write('simulation_number\twindow_name\twindow_type\tmax_chi_sq\n')
	for simulation_number in simulation_runs:
		print(simulation_number)
		# Extract causal elements
		causal_variant_file = simulated_trait_dir + 'simulation_' + str(simulation_number) + '_' + global_simulation_name_string + '_non_mediated_variant_causal_effect_sizes.txt'
		...
		for line in f:
			...
			chi_sq = np.square(tgfm_data['gwas_beta']/tgfm_data['gwas_beta_se'])
			max_chi_sq = np.max(chi_sq)

			window_type = 'Null'
			if full_window_name in fp_dicti:
				window_type = 'False_positive'
			elif full_window_name in tp_dicti:
				window_type = 'True_positive'

			t.write(str(simulation_number) + '\t' + window_name + '\t' + window_type + '\t' + str(max_chi_sq) + '\n')
...
def compute_pips(phi):
	prod = np.ones(phi.shape[1])
	for row_iter in range(phi.shape[0]):
		prod = prod*(1.0 - phi[row_iter,:])
	pip = 1.0 - prod
	return pip

def compute_causal_snp_pip(simulation_runs,global_simulation_name_string, eqtl_sample_size, twas_method, ln_pi_method, simulated_trait_dir, simulated_tgfm_results_dir, simulated_gene_position_file, simulated_gene_expression_dir, simulated_tgfm_input_data_dir,ld_score_output_file, cs_coverage_per_high_pip_snp_output_file, n_eqtl, abs_correlation_threshold):
	# Extract false-positive simulation-windows
	f = open(cs_coverage_per_high_pip_snp_output_file)
	fp_dicti = {}
	head_count = 0
	...
	t = open(ld_score_output_file,'w')
	t.write('simulation_number\twindow_name\tcausal_snp_id\ttag_false_positive_gene\tPIP\n')
	for simulation_number in simulation_runs:
		print(simulation_number)
		# Extract causal elements
		causal_variant_file = simulated_trait_dir + 'simulation_' + str(simulation_number) + '_' + global_simulation_name_string + '_non_mediated_variant_causal_effect_sizes.txt'
		...
		for line in f:
			...
			# Get middle causal variatns
			middle_variants = tgfm_data['variants'][tgfm_data['middle_variant_indices']]
			middle_variant_indices = np.copy(tgfm_data['middle_variant_indices'])
			#middle_variants = tgfm_data['variants']
			#middle_variant_indices = np.arange(len(tgfm_data['variants']))


			middle_causal_variants = []
			middle_causal_variant_indices = []
			for itera in range(len(middle_variants)):
				if middle_variants[itera] in causal_variants:
					middle_causal_variants.append(middle_variants[itera])
					middle_causal_variant_indices.append(middle_variant_indices[itera])
			middle_causal_variants = np.asarray(middle_causal_variants)
			middle_causal_variant_indices = np.asarray(middle_causal_variant_indices)

			if len(middle_causal_variants) == 0:
				continue

			# Extract PIPs of middle causal variants
			tgfm_results_file = simulated_tgfm_results_dir + 'simulation_' + str(simulation_number) + '_' + global_simulation_name_string + '_eqtl_ss_' + str(eqtl_sample_size) + '_' + twas_method + '_ln_pi_uniform_init_susie_inf_resid_var_False_' + window_name + '_results.pkl'
			if os.path.exists(tgfm_results_file) == False:
				continue
			g = open(tgfm_results_file, "rb")
			tgfm_res = pickle.load(g)
			phi = np.copy(tgfm_res['beta_phi'])
			PIPs = compute_pips(phi[:, middle_causal_variant_indices])
			g.close()

			tag_false_positive_gene_arr = np.asarray(['False']*len(PIPs))

			# Window has false positive gene
			if full_window_name in fp_dicti:
				fp_genes_dicti = fp_dicti[full_window_name]
				ld_file = data[1]
				# Load in data
				ld_mat = np.load(ld_file)
				...
				full_ld = extract_full_gene_variant_ld(tgfm_data['gene_eqtl_pmces'], tgfm_data['reference_ld'])


				n_genes = len(tgfm_data['genes'])
				ld_genes_by_causal_variants = full_ld[:n_genes, (middle_causal_variant_indices + n_genes)]
				fp_gene_indices = []
				for ii, gene_name in enumerate(tgfm_data['genes']):
					if gene_name in fp_genes_dicti:
						fp_gene_indices.append(ii)
				fp_gene_indices = np.asarray(fp_gene_indices)

				final_ld = ld_genes_by_causal_variants[fp_gene_indices,:]

				correlated_causal_variants = np.max(np.abs(final_ld),axis=0) > abs_correlation_threshold

				tag_false_positive_gene_arr[correlated_causal_variants] = 'True'

			for variant_iter in range(len(middle_causal_variants)):
				variant_name = middle_causal_variants[variant_iter]
				pip = PIPs[variant_iter]
				tag_fp = tag_false_positive_gene_arr[variant_iter]
				t.write(str(simulation_number) + '\t' + window_name + '\t' + variant_name + '\t' + tag_fp + '\t' + str(pip) + '\n')
		f.close()
		t.flush()
	t.close()
...
initialization_versions = np.asarray(['best', 'susie_inf'])
resid_var_versions = np.asarray(['False'])
n_eqtl_per_gene = np.asarray(['2', '4', '6', '8', '10'])
...
##############################
n_eqtl='6'
organized_number_causal_elements_per_region_output_file = simulated_organized_results_dir + 'organized_simulation_' + global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl + '_number_of_causal_genetic_elements_per_region.txt'
#create_file_containing_number_of_causal_genetic_elements_per_region(global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl, simulation_runs, simulated_gene_expression_dir, simulated_trait_dir, organized_number_causal_elements_per_region_output_file, bim_file, simulated_gene_position_file)
...
for pip_threshold in pip_thresholds:
	...
	#create_file_containing_averaged_tgfm_high_pip_calibration(cs_coverage_per_high_pip_snp_output_file, cs_high_pip_coverage_output_file, eqtl_sample_sizes, ln_pi_methods, initialization_versions, twas_methods, resid_var_versions, n_eqtl_per_gene)
...
for pip_threshold in pip_thresholds:
	...
	#create_file_containing_averaged_tgfm_cs_power(cs_power_per_component_output_file, cs_power_output_file, eqtl_sample_sizes, ln_pi_methods, initialization_versions,twas_methods, resid_var_versions, n_eqtl_per_gene)
...
##################################
# Compute chi-squared stats of of false positive windows vs tp positive windows
##################################
...
max_chi_squared_per_window_output = simulated_organized_results_dir + 'organized_simulation_' + global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl + '_tgfm_' + str(eqtl_sample_size) + '_' + twas_method + '_max_chi_squared_per_window.txt'
#compute_max_chi_squared_per_window(simulation_runs, global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl, eqtl_sample_size, twas_method, ln_pi_method, simulated_trait_dir, simulated_tgfm_results_dir, simulated_gene_position_file, simulated_gene_expression_dir, simulated_tgfm_input_data_dir,max_chi_squared_per_window_output, cs_coverage_per_high_pip_snp_output_file, n_eqtl)

##################################
# Compare PIPs of causal snps with PIPs of causal snps correlated with false positive gene
##################################
eqtl_sample_size='inf'
twas_method = 'susie_pmces'
ln_pi_method = 'uniform'
...
causal_snp_pip_output = simulated_organized_results_dir + 'organized_simulation_' + global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl + '_tgfm_' + str(eqtl_sample_size) + '_' + twas_method + '_causal_snp_pip.txt'
compute_causal_snp_pip(simulation_runs, global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl, eqtl_sample_size, twas_method, ln_pi_method, simulated_trait_dir, simulated_tgfm_results_dir, simulated_gene_position_file, simulated_gene_expression_dir, simulated_tgfm_input_data_dir,causal_snp_pip_output, cs_coverage_per_high_pip_snp_output_file, n_eqtl, float(correlation_threshold))
...
ld_score_output_file = simulated_organized_results_dir + 'organized_simulation_' + global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl + '_tgfm_' + str(eqtl_sample_size) + '_' + twas_method + '_gene_causal_genetic_element_ld_scores.txt'
...
cs_coverage_per_high_pip_snp_output_file =  simulated_organized_results_dir + 'organized_simulation_' + global_simulation_name_string + '_tgfm_inf_eqtl_pip_' + str(pip_threshold) + '_calibration_per_component.txt'
directional_ld_sums_output_file = simulated_organized_results_dir + 'organized_simulation_' + global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl + '_tgfm_' + str(eqtl_sample_size) + '_' + twas_method + '_gene_causal_genetic_element_directional_ld_sum.txt'
compute_directional_ld_sums_between_genes_and_causal_genetic_elements(simulation_runs, global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl, eqtl_sample_size, twas_method, ln_pi_method, simulated_trait_dir, simulated_tgfm_results_dir, simulated_gene_position_file, simulated_gene_expression_dir, simulated_tgfm_input_data_dir,directional_ld_sums_output_file, cs_coverage_per_high_pip_snp_output_file, n_eqtl)
...
cs_coverage_per_high_pip_snp_output_file =  simulated_organized_results_dir + 'organized_simulation_' + global_simulation_name_string + '_tgfm_inf_eqtl_pip_' + str(pip_threshold) + '_calibration_per_component.txt'
ld_score_output_file = simulated_organized_results_dir + 'organized_simulation_' + global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl + '_tgfm_' + str(eqtl_sample_size) + '_' + twas_method + '_identified_gene_identified_non_mediated_variant_ld_scores.txt'
compute_ld_scores_between_identified_genes_and_identified_non_mediated_variants(simulation_runs, global_simulation_name_string, global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl, eqtl_sample_size, twas_method, ln_pi_method, simulated_trait_dir, simulated_tgfm_results_dir, simulated_gene_position_file, simulated_gene_expression_dir, simulated_tgfm_input_data_dir,ld_score_output_file, cs_coverage_per_high_pip_snp_output_file, n_eqtl)




##################################
# Compute LD scores between identified genes and variant model un-identified causal variants
##################################
eqtl_sample_size='inf'
twas_method = 'susie_pmces'
ln_pi_method = 'uniform'
...
cs_coverage_per_high_pip_snp_output_file =  simulated_organized_results_dir + 'organized_simulation_' + global_simulation_name_string + '_tgfm_inf_eqtl_pip_' + str(pip_threshold) + '_calibration_per_component.txt'
ld_score_output_file = simulated_organized_results_dir + 'organized_simulation_' + global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl + '_tgfm_' + str(eqtl_sample_size) + '_' + twas_method + '_identified_gene_unidentified_causal_non_mediated_variant_ld_scores.txt'
compute_ld_scores_between_identified_genes_and_unidentified_causal_non_mediated_variants(simulation_runs, global_simulation_name_string, global_simulation_name_string + '_n_eqtl_per_gene_' + n_eqtl, eqtl_sample_size, twas_method, ln_pi_method, simulated_trait_dir, simulated_tgfm_results_dir, simulated_gene_position_file, simulated_gene_expression_dir, simulated_tgfm_input_data_dir,ld_score_output_file, cs_coverage_per_high_pip_snp_output_file, n_eqtl)
