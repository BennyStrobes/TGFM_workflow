...
def make_ld_score_input_shell(chrom_num,  n_genes_per_pseudotissue, pseudotissue_names, variant_models, gene_model_suffixes, preprocessed_tgfm_sldsc_data_dir, gene_type, tissue_version):
	for variant_model in variant_models:
		...
		for gene_model_suffix in gene_model_suffixes:
			...
			# Create output root
			output_root = preprocessed_tgfm_sldsc_data_dir + 'sc_' + variant_model + '_' + gene_type + '_' + tissue_version + '_' + gene_model_suffix + '.' + str(chrom_num) + '.l2'
...
def make_ld_score_input_shell_for_genotype_intercept(chrom_num, n_genes_per_pseudotissue, pseudotissue_names, reference_variant_model, gene_model_suffixes, preprocessed_tgfm_sldsc_data_dir, gene_type, tissue_version):
	...
	for gene_model_suffix in gene_model_suffixes:
		...
		# Create output root
		output_root = preprocessed_tgfm_sldsc_data_dir + 'sc_' + 'genotype_intercept' + '_' + gene_type + '_' + tissue_version + '_' + gene_model_suffix + '.' + str(chrom_num) + '.l2'
...
def get_cell_type_names(pb_cell_type_file):
...
preprocessed_tgfm_sldsc_data_dir = sys.argv[3]
gene_type = sys.argv[4]
gtex_susie_gene_models_dir = sys.argv[5]
pb_susie_gene_models_dir = sys.argv[6]
tissue_version = sys.argv[7]


# Extract names of pseudotissues
pseudotissue_gtex_only_names = get_pseudotissue_names(gtex_pseudotissue_file, tissue_version)
cell_type_names = get_cell_type_names(pb_cell_type_file)


# Get number of genes per chromosome per tissue
n_genes_per_chromosome_per_tissue_gtex = get_n_genes_per_chromosome_per_tissue(pseudotissue_gtex_only_names, gtex_susie_gene_models_dir, gene_type)
n_genes_per_chromosome_per_tissue_sc = get_n_genes_per_chromosome_per_tissue(cell_type_names, pb_susie_gene_models_dir, gene_type)

n_genes_per_chromosome_per_tissue = np.hstack((n_genes_per_chromosome_per_tissue_gtex, n_genes_per_chromosome_per_tissue_sc))

pseudotissue_names = np.hstack((pseudotissue_gtex_only_names, cell_type_names))
...
for variant_model in variant_models:
	...
	non_neg_coef_file = preprocessed_tgfm_sldsc_data_dir + 'sc_' + variant_model + '_' + tissue_version + '_nonnegative_coefficients.txt'
...
non_neg_coef_file = preprocessed_tgfm_sldsc_data_dir + 'sc_' + 'genotype_intercept' + '_' + tissue_version + '_nonnegative_coefficients.txt'
t = open(non_neg_coef_file,'w')
n_gene_annotations = len(pseudotissue_names)
t.write('False\n')
for gene_iter in range(n_gene_annotations):
