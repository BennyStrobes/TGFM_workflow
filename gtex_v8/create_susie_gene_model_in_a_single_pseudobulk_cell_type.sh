...
gene_model_input_dir="$3"
gene_model_output_dir="$4"
job_number="$5"
...
gene_summary_file=$gene_model_input_dir$pseudotissue_name"/cell_types_"$pseudotissue_name"_fusion_ready_gene_summary_"$job_number".txt"

# Output stem
tissue_gtex_fusion_weights_dir=$gene_model_output_dir$pseudotissue_name"/"
...
sed 1d $gene_summary_file | while read gene_id chrom_num tss composit_gene_pheno_file composit_covariate_file composit_tissue_string; do
	...
	for tissue_index in "${!composit_tissue_arr[@]}"; do
		...
		plink --bfile $gene_model_input_dir$pseudotissue_name"/cell_types_"$pseudotissue_name"_genotype_chr_"${chrom_num} --keep-allele-order --pheno $gene_pheno_file --make-bed --out $OUT"_"$composit_tissue --keep $gene_pheno_file --chr $chrom_num --from-bp $p0 --to-bp $p1 --allow-no-sex
	done
