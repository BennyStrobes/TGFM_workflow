...
# Bootstrapped nonnegative SLDSC
class BS_NN_SLDSC(object):

	__null_intercept__ = 1

	def __init__(self, y, x, w, N, M, n_blocks=200, n_bootstraps=100, nonnegative_coefficients=[None], intercept=None, old_weights=False):
		# Error checks and what not
		for i in [y, x, w, M, N]:
			try:
				if len(i.shape) != 2:
		...
		# Deal with nonnegative coefficients
		if nonnegative_coefficients[0] is None:
			# Set all coefficients to be unconstrained
			nonnegative_coefficients = np.asarray([False]*self.n_annot)
		else:
			# Error check here
			if len(nonnegative_coefficients) != self.n_annot:
		...
		if not self.constrain_intercept:
			...
			yp = y
			nonnegative_coefficients = np.hstack((nonnegative_coefficients, [False]))
		else:
			yp = y - intercept
			self.intercept_se = 'NA'
			print('currently not implemented for this method: assumption eroorro')
			pdb.set_trace()
		del y
		...
		bstrap = bs.partiallyNonNegLstsqBStrap(x, y, n_blocks, nonnegative_coefficients, n_bootstraps)
		self.bootstrapped_coefs = (bstrap.bootstrapped_coefs[:, :self.n_annot])/Nbar
		self.mean_bootstrapped_coefs = np.mean(self.bootstrapped_coefs,axis=0)
		self.se_bootstrapped_coefs = np.std(self.bootstrapped_coefs,axis=0,ddof=1)
		self.global_coefs = (bstrap.global_coef[:self.n_annot])/Nbar
