...
def edgeR_cpm(counts_df, tmm=None, normalized_lib_sizes=True, log=False, pseudocount=1):
    """
    ...
    if log == True:
        scaled_pseudocounts = pseudocount*(lib_size/np.mean(lib_size))
        new_lib_size = lib_size + (2.0*scaled_pseudocounts)
        normalized = np.log2((counts_df + scaled_pseudocounts) / new_lib_size * 1e6)
    else: 
        normalized = counts_df / lib_size * 1e6
    return normalized