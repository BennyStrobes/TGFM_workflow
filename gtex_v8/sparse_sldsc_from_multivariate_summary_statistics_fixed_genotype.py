...
class SPARSE_SLDSC_FIXED_TERM(object):
	...
	def update_susie_effects(self, expected_log_pi, component_variances):
		# Update genotype component variance

		# Include current effect (as it is currently removed)
		self.residual[0] = self.residual[0] + self.genotype_beta_mu

		# Compute variational updates
		b_term = np.dot(self.residual,self.tau_cov_inv)[0]
		a_term = (-.5*self.tau_cov_inv[0,0]) - .5*(1.0/self.genotype_component_variance)
		self.genotype_beta_var = -1.0/(2.0*a_term)
		self.genotype_beta_mu = b_term*self.genotype_beta_var
		...
		for l_index in range(self.L):
			...
			self.residual[1:] = self.residual[1:] + component_pred


			#################
			# Beta effects
			#################
			beta_b_terms = np.dot(self.residual,self.tau_cov_inv)[1:]
			beta_a_terms = (-.5*np.diag(self.tau_cov_inv)[1:]) - .5*(1.0/component_variances[l_index])
	...
	def update_component_variances(self):
		...
		self.genotype_component_variance = np.square(self.genotype_beta_mu) + self.genotype_beta_var
	...
	def initialize_variables(self, tau, tau_cov):
		...
		# Initialize genotype component variance
		self.genotype_component_variance = 1.0
		...
		self.genotype_beta_mu = 0.0
		self.genotype_beta_var = 1.0
