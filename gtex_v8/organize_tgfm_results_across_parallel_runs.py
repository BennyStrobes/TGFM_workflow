...
def generate_per_gene_tissue_pip_full_summary_file(concatenated_pip_summary_file, per_gene_tissue_full_pip_summary_file, tissue_name_to_broad_category, data_file_stem, file_stem, model_version, processed_tgfm_input_stem):
	...
	for line in f:
		...
		for ii, gene_tissue_name in enumerate(middle_gene_names):
			...
			t.write(gene_tissue_name + '\t' + gene_name + '\t' + tissue_name + '\t' + tissue_category + '\t' + window_name + '\t' + str(gene_tissue_pip) + '\n')
	f.close()
	t.close()
	return
...
tgfm_organized_results_dir = sys.argv[10]
gene_annotation_file = sys.argv[11]
...
for trait_name in trait_names:
	...
	for model_version in model_versions:
		...
		#generate_per_gene_tissue_pip_full_summary_file(concatenated_pip_summary_file, per_gene_tissue_full_pip_summary_file, tissue_name_to_broad_category, data_file_stem, file_stem, model_version, processed_tgfm_input_stem)
		...
		###################################################
		# Tally up number of causal genetic elements into 5 sqaure root bins
		###################################################
		n_causal_genetic_elements_summary_cross_threshold_file = file_stem + '_tgfm_n_causal_genetic_elements_5_pip_bins_sqrt_plot_input.txt'
		sqrt_root_bins = [1.0, .9, .7, .5, .3, .2]
		#tally_number_of_causal_genetic_elements_sqrt_plot_input(concatenated_pip_summary_file, n_causal_genetic_elements_summary_cross_threshold_file, sqrt_root_bins)



		###################################################
		# Create gene-alt_tissue pip summary file
		###################################################
		per_gene_alttissue_pip_summary_file = file_stem + '_tgfm_per_gene_alttissue_pip_summary.txt'
		#generate_per_gene_alttissue_pip_summary_file(concatenated_pip_summary_file, per_gene_alttissue_pip_summary_file, tissue_name_to_alt_tissue, data_file_stem, file_stem, model_version, processed_tgfm_input_stem)
		...
		###################################################
		for pip_threshold in [0.0, .1, .2, .25]:
			causal_tissue_pvalue_file = file_stem + '_tgfm_causal_tissue_pvalues_ignore_pip_less_than_' + str(pip_threshold) + '.txt'
			#compute_causal_tissue_pvalues(concatenated_pip_summary_file, causal_tissue_pvalue_file, tissue_names, pip_threshold=pip_threshold)

		###################################################
		# Create component level summary data
		###################################################
		component_level_summary_file = file_stem + '_tgfm_component_level_summary.txt'
		#generate_component_level_summary_data(concatenated_pip_summary_file, component_level_summary_file, tissue_names, data_file_stem,file_stem, model_version, processed_tgfm_input_stem)
		...
		###################################################
		tissue_overlap_jaccard_file = file_stem + '_tgfm_tissue_overlap_jaccard.txt'
		#compute_tgfm_tissue_overlap_jaccard(concatenated_pip_summary_file, tissue_overlap_jaccard_file, tissue_names)
		...
		#generate_per_tissue_category_pip_summary_file(concatenated_pip_summary_file, per_tissue_category_pip_summary_file, tissue_names,tissue_categories, tissue_category_to_tissue_names, data_file_stem, file_stem, model_version, processed_tgfm_input_stem)
