...
def compute_odds_ratio_quantities_for_this_pathways(gene_to_tgfm_gene_pip, pathway_ensamble_ids, all_tgfm_gene_dicti, pip_threshold=.5):
	...
	ensamble_ids = np.asarray([*all_tgfm_gene_dicti])
	for ensamble_id in ensamble_ids:
		if ensamble_id in gene_to_tgfm_gene_pip:
...
def compute_probabilistic_odds_ratio_quantities_for_this_pathways(gene_to_tgfm_gene_pip, pathway_ensamble_ids, all_tgfm_gene_dicti, pip_threshold=.5):
	# Pathway ensamble id dicti
	pathway_ensamble_id_dicti = {}
	for pathway_ensamble_id in pathway_ensamble_ids:
		pathway_ensamble_id_dicti[pathway_ensamble_id.split('.')[0]] = 1
...
tgfm_genes = np.loadtxt(tgfm_gene_list_file,dtype=str,delimiter='\t')
all_tgfm_gene_dicti = {}
for tgfm_gene in tgfm_genes[1:]:
	all_tgfm_gene_dicti[tgfm_gene.split('.')[0]] = 1
...
for trait_name in independent_traits:
	for pip_threshold in pip_thresholds:
		...
		t = open(biological_pathway_enrichment_dir + 'biologial_pathway_enrichment_analysis_2_' + trait_name + '_' + str(pip_threshold) + '.txt','w')
		...
		for line in f:
			...
			aa, bb, cc, dd = compute_odds_ratio_quantities_for_this_pathways(gene_to_tgfm_gene_pip, pathway_ensamble_ids, all_tgfm_gene_dicti,pip_threshold=pip_threshold)
...
for trait_name in independent_traits:
	for pip_threshold in pip_thresholds:
		...
		# Open output file handle for this trait
		t = open(biological_pathway_enrichment_dir + 'biologial_pathway_enrichment_analysis_2_' + trait_name + '_probabability_weighted_' + str(pip_threshold) + '.txt','w')
		t.write('pathway_index\tpathway_name\taa\tbb\tcc\tdd\torat\torat_pvalue\n')
		# Loop through pathways and do enrichment for each
		f = open(biological_pathway_gene_set_file)
		head_count = 0
		for line in f:
			...
			pathway_name = data[1]
			pathway_ensamble_ids = np.asarray(data[5].split(','))
			aa, bb, cc, dd = compute_probabilistic_odds_ratio_quantities_for_this_pathways(gene_to_tgfm_gene_pip, pathway_ensamble_ids, all_tgfm_gene_dicti,pip_threshold=pip_threshold)
			fe_orat, fe_pvalue = scipy.stats.fisher_exact(np.asarray([[aa,bb],[cc,dd]]))		
			t.write(pathway_index + '\t' + pathway_name + '\t' + str(aa) + '\t' + str(bb) + '\t' + str(cc) + '\t' + str(dd) + '\t' + str(fe_orat) + '\t' + str(fe_pvalue) + '\n')
		f.close()
		t.close()
