...
def generate_component_level_abf_summary_data(concatenated_pip_summary_file, component_level_abf_summary_file, tissue_names, file_stem, model_version, processed_tgfm_input_stem, per_window_abf_output_stem):
	...
	t.write('window_name\tbootstrap_number\tcomponent_number\telement_names\twindow_abf_file\twindow_component_samples_file\tannotation_mat_file\n')
		...
		if model_version.startswith('pmces'):
			for component_iter in tgfm_results['valid_components']:
				if component_in_middle_of_window(tgfm_results['alpha_phi'][component_iter, :], tgfm_results['beta_phi'][component_iter, :], middle_gene_indices_dicti, middle_variant_indices_dicti):
					...
					t.write(window_name + '\t' + 'NA' + '\t' + str(component_iter) + '\t' + ';'.join(element_names) + '\t' + per_window_abf_output_file + '\t' + per_window_sample_names_output_file + '\t' + per_window_anno_output_file + '\n')
			...
			for sample_iter in range(n_samples):
				...
				for component_iter in sample_valid_components:
					if component_in_middle_of_window(tgfm_results['alpha_phis'][component_iter][sample_iter,:], tgfm_results['beta_phis'][component_iter][sample_iter,:], middle_gene_indices_dicti, middle_variant_indices_dicti):
						...
						element_names = np.hstack((tgfm_results['genes'], tgfm_results['variants']))
						t.write(window_name + '\t' + str(sample_iter) + '\t' + str(component_iter) + '\t' + ';'.join(element_names) + '\t' + per_window_abf_output_file + '\t' + per_window_sample_names_output_file + '\t' + per_window_anno_output_file+ '\n')
						per_window_abf.append(log_abf)
						window_has_component = True
...
def learn_iterative_variant_gene_tissue_prior_pip_level_bootstrapped(component_level_abf_summary_file, tgfm_version, tissue_names, per_window_abf_output_stem, max_iter=60, n_bootstraps=200):
	# Create mapping from tissue name to tissue position
	tissue_name_to_position = {}
	for ii, tissue_name in enumerate(tissue_names):
		tissue_name_to_position[tissue_name] = ii
...
def learn_iterative_anno_variant_gene_tissue_prior_pip_level_bootstrapped(component_level_abf_summary_file, tgfm_version, tissue_names, per_window_abf_output_stem, max_iter=60, n_bootstraps=200):
	...
	for itera in range(max_iter):
		print(itera)
		old_tissue_probs = np.copy(tissue_probs_distr)
		variant_prob_distr, tissue_probs_distr = update_prior_prob_for_anno_variant_gene_tissue_bootstrapped(component_level_abf_summary_file, tgfm_version, tissue_name_to_position, variant_prob_distr, tissue_probs_distr, tissue_names, window_to_class_to_indices, n_bootstraps, bs_indices, bs_mapping, window_names, window_to_bootstraps, n_anno)
		diff=tissue_probs_distr - old_tissue_probs
		print(np.sort(np.mean(diff,axis=1)))
	...
	for line in f:
		...
		if window_name != prev_window_name:
			if prev_window_name == 'NULL':
				...
				# update global counts
				# For variants
				indices = np.asarray(window_to_class_to_indices[prev_window_name]['variant'])
				...
				# For genes
				for tiss_iter, tissue_name in enumerate(tissue_names):
					indices = np.asarray(window_to_class_to_indices[prev_window_name][tissue_name])
					if len(indices) == 0:
						continue
...
def update_prior_prob_for_anno_variant_gene_tissue_bootstrapped(component_level_abf_summary_file, tgfm_version, tissue_name_to_position, variant_prob_distr, tissue_probs_distr, tissue_names, window_to_class_to_indices, n_bootstraps, bs_indices, bs_mapping, window_names, window_to_bootstraps, n_anno):
	# General info
	n_tissues = len(tissue_names)	
	n_windows = len(window_names)
	...
	for line in f:
		...
		if window_name != prev_window_name:
			if prev_window_name == 'NULL':
				...
				window_variant_anno_arr.append(window_anno)
				#variant_posterior_sum = variant_posterior_sum + (bs_scaling_factors*np.sum(expected_pips[indices,:],axis=0))
				#variant_counts = variant_counts + (bs_scaling_factors*len(indices))
				# For genes
				for tiss_iter, tissue_name in enumerate(tissue_names):
	...
	for bs_iter in range(n_bootstraps):
		...
		coefs = manual_logistic_regression(bs_anno,bs_pip, variant_prob_distr[:, bs_iter], fraction=.1)
...
###################################################
n_bootstraps=100
variant_prob_emperical_distr, tissue_probs_emperical_distr = learn_iterative_anno_variant_gene_tissue_prior_pip_level_bootstrapped(component_level_abf_summary_file, tgfm_version, tissue_names, per_window_abf_output_stem, max_iter=30, n_bootstraps=n_bootstraps)
...
###################################################
n_bootstraps=100
variant_prob_emperical_distr, tissue_probs_emperical_distr = learn_iterative_variant_gene_tissue_prior_pip_level_bootstrapped(component_level_abf_summary_file, tgfm_version, tissue_names, per_window_abf_output_stem, max_iter=200, n_bootstraps=n_bootstraps)
