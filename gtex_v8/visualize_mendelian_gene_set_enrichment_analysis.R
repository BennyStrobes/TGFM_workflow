...
mean_se_barplot_of_prop_drug_target_gene_score_binned_by_tgfm_pip_and_twas_z2 <- function(df) {
	...
	threshold_lb <- 0.0
	threshold_ub <- .1
	indices = (df$sum_tgfm_pip >= threshold_lb) & (df$sum_tgfm_pip < threshold_ub)
	prop = mean(df$mendelian_gene[indices])
	prop_se = sqrt((prop*(1.0-prop))/length(df$mendelian_gene[indices]))
	pops_mean_vec <- c(pops_mean_vec, prop)
	pops_mean_se_vec <- c(pops_mean_se_vec, prop_se)
...
summary_df <- read.table(paste0(working_dir, "mendelian_gene_set_overlap_summary_file.txt"), header=TRUE)
...
output_file <- paste0(working_dir, "mean_se_barplot_prop_mendelian_gene_binned_by_tgfm_pip_x_trait.pdf")
...
output_file <- paste0(working_dir, "mean_se_barplot_prop_mendelian_gene_binned_by_tgfm_pip_and_twas_z_x_trait.pdf")
