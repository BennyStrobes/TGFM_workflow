...
def create_alpha_mu_and_alpha_var_objects(twas_pickle_file_names, tissue_to_position_mapping):
	...
	return alpha_mu_object, alpha_var_object, beta_mu_object, beta_var_object, tissue_object, valid_gene_object, valid_variant_object, np.asarray(twas_obj_file_names)
...
def update_alpha_and_beta_in_a_single_window(alpha_mu_w, alpha_var_w, beta_mu_w, beta_var_w, tissue_object_w, twas_obj_file_name_w, expected_gamma_alpha, expected_gamma_beta):
	...
	for g_index in range(twas_obj['G']):
		gene_trait_pred_new = twas_obj['gene_eqtl_pmces'][g_index]*alpha_mu_w[g_index]
		temp_gene_pred = temp_gene_pred - gene_trait_pred_new
	residual = residual + np.dot(twas_obj['srs_inv'], (temp_gene_pred-beta_mu_w))  # Beta_mu remove predicted effect of genotype

	##########################
	# Update alpha and beta in this window
	#########################
	# Perform VI UPDATES on alpha_mu and alpha_var
	alpha_mu_w, alpha_var_w, residual = rss.update_alpha(alpha_mu_w, alpha_var_w, residual, twas_obj['gene_eqtl_pmces'], twas_obj['srs_inv'], twas_obj['G'], twas_obj['s_inv_2_diag'], twas_obj['precomputed_a_terms'], component_gamma_alpha)

	# Perform VI UPDATES on beta_mu and beta_var
	beta_mu_w, beta_var_w, residual = rss.update_beta(beta_mu_w, beta_var_w, residual, twas_obj['srs_inv'], twas_obj['K'], twas_obj['D_diag'], twas_obj['s_inv_2_diag'], expected_gamma_beta)
...
def update_alphas_and_betas(alpha_mu_object, alpha_var_object, beta_mu_object, beta_var_object, tissue_object, twas_obj_file_names, expected_gamma_alpha, expected_gamma_beta):
	...
	if parr == False:
		...
		for nn in range(num_windows):
			window_update_data.append(update_alpha_and_beta_in_a_single_window(alpha_mu_object[nn], alpha_var_object[nn], beta_mu_object[nn], beta_var_object[nn], tissue_object[nn], twas_obj_file_names[nn], expected_gamma_alpha, expected_gamma_beta))
	elif parr == True:
		window_update_data = Parallel(n_jobs=20)(delayed(update_alpha_and_beta_in_a_single_window)(alpha_mu_object[nn], alpha_var_object[nn], beta_mu_object[nn], beta_var_object[nn], tissue_object[nn], twas_obj_file_names[nn], expected_gamma_alpha, expected_gamma_beta) for nn in range(num_windows))
...
def infer_rss_likelihood_genome_wide_heritabilities(ordered_tissue_names, twas_pickle_file_names, temp_alpha_output_file, temp_beta_output_file, max_iter=600):
	# Number of tissues
	...
	alpha_mu_object, alpha_var_object, beta_mu_object, beta_var_object, tissue_object, valid_gene_object, valid_variant_object, twas_obj_file_names = create_alpha_mu_and_alpha_var_objects(twas_pickle_file_names, tissue_to_position_mapping)
...
window_names = np.random.choice(window_names,1000,replace=False)
...
expected_gamma_alpha, gamma_alpha_a, gamma_alpha_b, gamma_beta_a, gamma_beta_b = infer_rss_likelihood_genome_wide_heritabilities(ordered_tissue_names, window_pickle_file_names, temp_alpha_output_file, temp_beta_output_file)
