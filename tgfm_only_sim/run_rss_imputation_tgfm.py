...
def standardize_eqtl_pmces_old(eqtl_pmces, variant_ld, gene_noise_ratios):

	total_gene_variances = np.copy(gene_noise_ratios)

	#gene_variances = np.diag(np.dot(np.dot(eqtl_pmces, variant_ld), np.transpose(eqtl_pmces)))
	gene_pmces_variances = np.sum(np.dot(eqtl_pmces, variant_ld)*eqtl_pmces, axis=1)
	...
	for gene_iter in range(n_genes):
		eqtl_pmces[gene_iter,:] = eqtl_pmces[gene_iter,:]/np.sqrt(gene_pmces_variances[gene_iter])

		gene_residual_noise = total_gene_variances[gene_iter] - gene_pmces_variances[gene_iter]


		gene_noise_ratios[gene_iter] = gene_residual_noise/gene_pmces_variances[gene_iter]

	gene_noise_ratios[gene_noise_ratios > 1.0] = 1.0
	# Note that gene noise ratios are 1.0/F-statistic

	return eqtl_pmces, gene_noise_ratios
...
def tgfm_inference_shell(tgfm_data, gene_log_prior, var_log_prior, gene_variant_full_ld, init_method, est_resid_var_bool, tmp_output_filestem):
	...
	if init_method == 'best':
		# Extract gene_standard errors
		gene_se = np.asarray([np.mean(tgfm_data['gwas_beta_se'])]*len(new_gene_z))
		gene_beta = new_gene_z*gene_se

		# Get concatenated vector of beta and beta standard errors
		full_beta = np.hstack((gene_beta, tgfm_data['gwas_beta']))
		full_beta_se = np.hstack((gene_se, tgfm_data['gwas_beta_se']))

		# Extract vector of gene noise ratios
		gene_noise_ratios = np.zeros(len(full_beta))
		gene_noise_ratios[:len(new_gene_z)] = tgfm_data['gene_noise_ratios']

		# non imputation (standard version)
		#standard_susie = susie_alt.SUSIE_ALT(L=20, estimate_prior_variance=True, estimate_prior_prob=False)
		#standard_susie.fit(full_beta, full_beta_se, gene_variant_full_ld)

		# Imputation version
		impute_susie = susie_impute.SUSIE_IMPUTE(L=20, estimate_prior_variance=True)
		impute_susie.fit(full_beta, full_beta_se, gene_variant_full_ld, gene_noise_ratios)

		# Alphas
		susie_alpha = impute_susie.beta_phi
		tgfm_obj.alpha_phi = susie_alpha[:,:(tgfm_obj.G)]
		tgfm_obj.beta_phi = susie_alpha[:,(tgfm_obj.G):]

		# Mus
		susie_mu = impute_susie.beta_mu
		tgfm_obj.alpha_mu = susie_mu[:,:(tgfm_obj.G)]
		tgfm_obj.beta_mu = susie_mu[:,(tgfm_obj.G):]

		# susie_mu_var
		susie_mu_var = impute_susie.beta_var
		tgfm_obj.alpha_var = susie_mu_var[:,:(tgfm_obj.G)]
		tgfm_obj.beta_var = susie_mu_var[:,(tgfm_obj.G):]
...
for line in f:
	...
	tgfm_data['gene_noise_ratios'] = np.copy(tgfm_data['gene_variances'])
	tgfm_data['gene_eqtl_pmces'], tgfm_data['gene_noise_ratios'] = standardize_eqtl_pmces_old(tgfm_data['gene_eqtl_pmces'], tgfm_data['reference_ld'], tgfm_data['gene_noise_ratios'])
