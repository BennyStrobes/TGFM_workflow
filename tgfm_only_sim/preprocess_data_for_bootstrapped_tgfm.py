...
def extract_gene_tissue_pairs_and_associated_gene_models_in_window(window_start, window_end, gene_summary_file, simulated_learned_gene_models_dir, simulation_name_string, n_causal_genetic_elements_str, eqtl_sample_size, window_indices, simulated_gene_expression_dir, ld_mat, eqtl_type, genotype_obj, n_bs):
	...
	for line in f:
		...
		if gene_tss >= (window_start + 100000) and gene_tss <= (window_end - 100000):
			...
			# Fitted gene file
			if eqtl_type == 'marginal_distr':
				fitted_gene_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + n_causal_genetic_elements_str + '_' + ensamble_id + '_eqtlss_' + str(eqtl_sample_size) + '_marginal_effects_gene_model.npy'
				marginal_effects_mat = np.load(fitted_gene_file)
				# Extract sparse genotype for this
				pruned_svd_lambda, pruned_svd_Q, pruned_svd_ld_inv_mat = extract_sparse_genotype_matrix_for_this_gene_from_ld(ld_mat[cis_snp_indices[window_indices],:][:,cis_snp_indices[window_indices]], float(eqtl_sample_size))
				#pruned_svd_lambda2, pruned_svd_Q2, pruned_svd_ld_inv_mat2 = extract_sparse_genotype_matrix_for_this_gene(genotype_obj, cis_snp_indices, float(eqtl_sample_size))
				gene_model_mat = np.transpose(np.dot(pruned_svd_ld_inv_mat, np.transpose(marginal_effects_mat)))
			elif eqtl_type == 'sparse_marginal_distr':
				fitted_gene_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + n_causal_genetic_elements_str + '_' + ensamble_id + '_eqtlss_' + str(eqtl_sample_size) + '_marginal_effects_gene_model.npy'
				marginal_effects_mat = np.load(fitted_gene_file)
				# Extract sparse genotype for this
				pruned_svd_lambda, pruned_svd_Q, pruned_svd_ld_inv_mat = extract_sparse_genotype_matrix_for_this_gene_from_ld(ld_mat[cis_snp_indices[window_indices],:][:,cis_snp_indices[window_indices]], float(eqtl_sample_size))
				...
				fitted_gene_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + n_causal_genetic_elements_str + '_' + ensamble_id + '_eqtlss_' + str(eqtl_sample_size) + '_gene_model_pmces.npy'
			...
			for tiss_iter in range(n_tiss):
				...
				if eqtl_type == 'marginal_distr':
					bs_marginal_effects_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + n_causal_genetic_elements_str + '_' + ensamble_id + '_eqtlss_' + str(eqtl_sample_size) + '_tissue_' + str(tiss_iter) + '_marginal_effects_bootstrapped.npy'
					bs_marginal_effects = np.load(bs_marginal_effects_file)[:n_bs,:]
					bs_alpha_effects = np.dot(pruned_svd_ld_inv_mat, np.transpose(bs_marginal_effects))
					...
					#gene_variance = calculate_gene_variance_according_to_marginal_distribution(marginal_effects_mat[tiss_iter,:], pruned_svd_ld_inv_mat, pruned_svd_lambda, pruned_svd_Q, float(eqtl_sample_size))
				elif eqtl_type == 'sparse_marginal_distr':
					bs_marginal_effects_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + n_causal_genetic_elements_str + '_' + ensamble_id + '_eqtlss_' + str(eqtl_sample_size) + '_tissue_' + str(tiss_iter) + '_marginal_effects_bootstrapped.npy'
					bs_marginal_effects = np.load(bs_marginal_effects_file)[:n_bs,:]
					bs_alpha_effects = np.dot(pruned_svd_ld_inv_mat, np.transpose(bs_marginal_effects))
					bs_gene_variances = np.diag(np.dot(np.dot(np.transpose(bs_alpha_effects), ld_mat[cis_snp_indices[window_indices],:][:,cis_snp_indices[window_indices]]), bs_alpha_effects))
					bs_delta_effects = np.dot(np.dot(np.diag(np.sqrt(pruned_svd_lambda)), np.transpose(pruned_svd_Q)), bs_alpha_effects)
					sparse_bs_delta_effects = np.copy(bs_delta_effects)*0.0
					for bs_iter in range(sparse_bs_delta_effects.shape[1]):
						indices_to_keep = np.argsort(-np.abs(bs_delta_effects[:,bs_iter]))[:5]
						sparse_bs_delta_effects[indices_to_keep, bs_iter] = bs_delta_effects[indices_to_keep, bs_iter]
					# Convert back
					sparse_bs_alpha_effects = np.dot(np.dot(pruned_svd_Q, np.diag(np.sqrt(1.0/pruned_svd_lambda))), sparse_bs_delta_effects)
					bs_gene_variances = np.diag(np.dot(np.dot(np.transpose(bs_alpha_effects), ld_mat[cis_snp_indices[window_indices],:][:,cis_snp_indices[window_indices]]), bs_alpha_effects))
					sparse_bs_gene_variances = np.diag(np.dot(np.dot(np.transpose(bs_alpha_effects), ld_mat[cis_snp_indices[window_indices],:][:,cis_snp_indices[window_indices]]), sparse_bs_alpha_effects))
					sparse_bs_std_alpha_effects = sparse_bs_alpha_effects/np.sqrt(sparse_bs_gene_variances)
					bs_arr.append((sparse_bs_std_alpha_effects, cis_snp_indices[window_indices]))
				elif eqtl_type == 'susie_distr':
	...
	return np.asarray(gene_tissue_pairs), bs_arr, np.asarray(gene_tss_arr)
...
n_causal_genetic_elements_str = sys.argv[14]
...
tgfm_input_data_summary_file = simulated_tgfm_input_data_dir + simulation_name_string + '_' + n_causal_genetic_elements_str + '_eqtl_ss_' + str(eqtl_sample_size) + '_' + eqtl_type + '_bootstrapped_tgfm_input_data_summary.txt'
...
for line in f:
	...
	# Extract LD
	ld_mat_file = simulated_tgfm_input_data_dir + simulation_name_string + '_' + n_causal_genetic_elements_str + '_' + window_name + '_in_sample_ld.npy'
	...
	gene_tissue_pairs, bs_eqtl_arr, gene_tissue_pairs_tss = extract_gene_tissue_pairs_and_associated_gene_models_in_window(window_start, window_end, gene_summary_file, simulated_learned_gene_models_dir, simulation_name_string, n_causal_genetic_elements_str,  eqtl_sample_size, window_indices, simulated_gene_expression_dir,ld_mat, eqtl_type, genotype_obj, n_bs)

	'''
	bs_gene_eqtl_files = []
	for bs_iter in range(n_bs):
		eqtl_mat = np.zeros((ld_mat.shape[0], len(gene_tissue_pairs)))
		for gene_itera, bs_eqtl_tuple in enumerate(bs_eqtl_arr):
			eqtl_gene_window = bs_eqtl_tuple[0][:, bs_iter]
			eqtl_mat[bs_eqtl_tuple[1],gene_itera] = eqtl_gene_window
		bs_eqtl_output_file = simulated_tgfm_input_data_dir + simulation_name_string + '_' + n_causal_genetic_elements_str + '_eqtl_ss_' + str(eqtl_sample_size) + '_' + eqtl_type + '_' + window_name + '_' + str(bs_iter) + '.npy'
		np.save(bs_eqtl_output_file, np.transpose(eqtl_mat))
	...
	window_gwas_summary_file = simulated_gwas_dir + simulation_name_string + '_' + n_causal_genetic_elements_str + '_simualated_gwas_results_window_' + window_name + '.txt'
	...
	ln_pi_output_stem = simulated_tgfm_input_data_dir + simulation_name_string + '_' + n_causal_genetic_elements_str + '_' + window_name + '_eqtl_ss_' + str(eqtl_sample_size)+ '_' + eqtl_type + '_ln_pi'
	# Uniform prior
	...
	tgfm_data['bs_gene_eqtl_pmces_files'] = bs_gene_eqtl_files
	...
	window_pickle_output_file = simulated_tgfm_input_data_dir + simulation_name_string + '_' + n_causal_genetic_elements_str + '_' + window_name + '_eqtl_ss_' + str(eqtl_sample_size)+ '_' + eqtl_type + '_tgfm_input_data.pkl'
