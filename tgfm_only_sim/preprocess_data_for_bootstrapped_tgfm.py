...
def extract_gene_tissue_pairs_and_associated_gene_models_in_window(window_start, window_end, gene_summary_file, simulated_learned_gene_models_dir, simulation_name_string, eqtl_sample_size, window_indices, simulated_gene_expression_dir, ld_mat, eqtl_type, genotype_obj, n_bs):
	...
	for line in f:
		...
		if gene_tss >= (window_start + 100000) and gene_tss <= (window_end - 100000):
			...
			if eqtl_type == 'marginal_distr':
				fitted_gene_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + ensamble_id + '_eqtlss_' + str(eqtl_sample_size) + '_marginal_effects_gene_model.npy'
			...
			for tiss_iter in range(n_tiss):
				...
				if eqtl_type == 'marginal_distr':
					bs_marginal_effects_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + ensamble_id + '_eqtlss_' + str(eqtl_sample_size) + '_tissue_' + str(tiss_iter) + '_marginal_effects_bootstrapped.npy'
...
for line in f:
	...
	gene_tissue_pairs, bs_eqtl_arr, gene_tissue_pairs_tss = extract_gene_tissue_pairs_and_associated_gene_models_in_window(window_start, window_end, gene_summary_file, simulated_learned_gene_models_dir, simulation_name_string, eqtl_sample_size, window_indices, simulated_gene_expression_dir,ld_mat, eqtl_type, genotype_obj, n_bs)
	...
	for bs_iter in range(n_bs):
		...
		for gene_itera, bs_eqtl_tuple in enumerate(bs_eqtl_arr):
			...
			#eqtl_mat.append(bs_eqtl_tuple[0][:, bs_iter])
