...
merged_gwas_file = simulated_gwas_dir + simulation_name_string + '_merged_gwas_summary_stats.txt'
mapping = {}
head_count = 0
f = open(merged_gwas_file)
for line in f:
	line = line.rstrip()
	data = line.split('\t')
	if head_count == 0:
	...
	string = rs_id + '\t' + str(beta) + '\t' + str(beta_se) + '\t' + str(beta_z)
	if rs_id in mapping:
		print('assumption eroror')
		pdb.set_trace()
	mapping[rs_id] = string
f.close()
