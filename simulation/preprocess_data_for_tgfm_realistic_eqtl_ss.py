...
def extract_gene_tissue_pairs_and_associated_gene_models_in_window(window_start, window_end, gene_summary_file, simulated_learned_gene_models_dir, simulation_name_string, window_indices, simulated_gene_expression_dir, ld_mat, eqtl_type, n_bs):
	...
	for line in f:
		...
		if gene_tss >= (window_start + 100000) and gene_tss <= (window_end - 100000):
			...
			if eqtl_type == 'susie':
				...
				gene_comp_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + ensamble_id + '_gene_valid_susie_comp.npy'
				gene_comp_arr = np.load(gene_comp_file)

				gene_best_to_worst_pi_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + ensamble_id + '_gene_best_to_worst_susie_pi_ratios.npy'
			...
			for tiss_iter in range(n_tiss):
				...
				if eqtl_type == 'susie':
					...
					gene_susie_mu_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + ensamble_id + '_tissue_' + str(tiss_iter) + '_gene_model_susie_mu.npy'
					gene_susie_alpha_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + ensamble_id + '_tissue_' + str(tiss_iter) + '_gene_model_susie_alpha.npy'
					gene_susie_mu_var_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + ensamble_id + '_tissue_' + str(tiss_iter) + '_gene_model_susie_mu_var.npy'
...
simulation_window_list_file = sys.argv[5]
annotation_file = sys.argv[6]
simulated_gwas_dir = sys.argv[7]
simulated_gene_expression_dir = sys.argv[8]
simulated_learned_gene_models_dir = sys.argv[9]
simulated_tgfm_input_data_dir = sys.argv[10]
eqtl_type = sys.argv[11]
...
tgfm_input_data_summary_file = simulated_tgfm_input_data_dir + simulation_name_string + '_' + eqtl_type + '_bootstrapped_tgfm_input_data_summary.txt'
...
for line in f:
	...
	gene_tissue_pairs, bs_eqtl_arr, gene_tissue_pairs_tss, pmces_weights, gene_variances, gene_susie_mus, gene_susie_mu_vars, gene_susie_alphas, gene_susie_indices, gene_valid_susie_comps, gene_best_to_worst_pi_ratios = extract_gene_tissue_pairs_and_associated_gene_models_in_window(window_start, window_end, gene_summary_file, simulated_learned_gene_models_dir, simulation_name_string, window_indices, simulated_gene_expression_dir,ld_mat, eqtl_type, n_bs)
	...
	window_pickle_output_file = simulated_tgfm_input_data_dir + simulation_name_string + '_' + window_name + '_' + eqtl_type + '_tgfm_input_data.pkl'
