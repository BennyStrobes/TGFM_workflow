...
def get_vector_of_nonnegative_coefficients(nonnegative_coefficient_file):
	if nonnegative_coefficient_file is None:
		nonnegative_coef = [None]
	else:
		f = open(nonnegative_coefficient_file)
		nonnegative_coef = []
		for line in f:
			line = line.rstrip()
			if line == 'True':
				nonnegative_coef.append(True)
			elif line == 'False':
				nonnegative_coef.append(False)
			else:
				print('assumption eororor')
				break
		f.close()

	return np.asarray(nonnegative_coef)
...
def estimate_h2(args, log):
	...
	if args.bootstrap == False:  
		...
		if args.print_delete_vals:
			...
			np.savetxt(args.out + '.intercept_delete', hsqhat.intercept_delete_values, delimiter='\t',fmt="%s")
		...
		nonnegative_coefficients = get_vector_of_nonnegative_coefficients(args.nonnegative_coefficient_file)
		hsqhat = reg.BS_NN_SLDSC(chisq, ref_ld, s(sumstats[w_ld_cname]), s(sumstats.N), M_annot, n_blocks=n_blocks, n_bootstraps=args.n_bootstraps, nonnegative_coefficients=nonnegative_coefficients, intercept=args.intercept_h2, old_weights=old_weights)
		
		annotation_names = np.asarray(ref_ld_cnames)

		# Print to output file
		t = open(args.out + 'sldsc_coefficients.txt','w')
		# Write header
		t.write('annotation_name\tglobal_tau\tmean_bootstrapped_tau\tstandard_error_bootstrapped_tau\tbootstrapped_taus\n')
		for annotation_iter, annotation_name in enumerate(annotation_names):
			t.write(annotation_name + '\t' + str(hsqhat.global_coefs[annotation_iter]) + '\t' + str(hsqhat.mean_bootstrapped_coefs[annotation_iter]) + '\t' + str(hsqhat.se_bootstrapped_coefs[annotation_iter]) + '\t' + ';'.join(hsqhat.bootstrapped_coefs[:,annotation_iter].astype(str)) + '\n')
		t.close()
