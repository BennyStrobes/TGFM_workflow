...
class TGFM(object):
	...
	def fit(self, twas_data_obj, variant_init_alpha, variant_init_mu):
		""" Fit the model.
			Args:
			twas_data_obj
		"""
		...
		self.initialize_susie_effects(self.gene_log_pi, self.variant_log_pi,self.component_variances, self.component_variances, variant_init_alpha, variant_init_mu)
	...
	def initialize_susie_effects(self, expected_log_pi, expected_log_variant_pi, alpha_component_variances, beta_component_variances, variant_init_alpha, variant_init_mu):
		# Loop through components
		for l_index in range(self.L):
			# Include current component (as it is currently removed)
			component_gene_trait_pred = np.dot(self.alpha_mu[l_index,:]*self.alpha_phi[l_index,:], self.gene_eqtl_pmces)
			...
			self.alpha_phi[l_index,:] = self.alpha_phi[l_index,:]*0.0
			self.alpha_mu[l_index,:] = self.alpha_mu[l_index,:]*0.0

			self.beta_phi[l_index,:] = variant_init_alpha[l_index,:]
			self.beta_mu[l_index,:] = variant_init_mu[l_index,:]
