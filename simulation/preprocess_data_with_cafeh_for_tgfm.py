...
def extract_cafeh_pmces_and_sampled_cafeh_distribution(cafeh_pi, cafeh_p_active, cafeh_mu, cafeh_mu_var, n_tiss, n_cis_snps, n_bs, p_active_thresh, max_min_pi_ratio_thresh):
	# Initialize PMCES
	pmces = np.zeros((n_tiss, n_cis_snps))

	# Initialize sampled effects
	bs_alpha_effects_vec = []
	for tt in range(n_tiss):
		bs_alpha_effects = np.zeros((n_cis_snps, n_bs))
		bs_alpha_effects_vec.append(bs_alpha_effects)
	n_comp = cafeh_pi.shape[0]

	for kk in range(n_comp):
		max_min_ratio = np.max(cafeh_pi[kk,:])/np.min(cafeh_pi[kk,:])
		if max_min_ratio < max_min_pi_ratio_thresh:
			continue
		# Randomly sample pi indices
		component_random_snp_indices = np.random.choice(np.arange(n_cis_snps).astype(int), replace=True, p=cafeh_pi[kk,:], size=n_bs)

		for tt in range(n_tiss):
			if cafeh_p_active[tt,kk] < p_active_thresh:
				continue
			# Get PMCES
			pmces[tt,:] = pmces[tt,:] + (cafeh_pi[kk,:]*cafeh_p_active[tt,kk]*cafeh_mu[tt][kk,:])

			# Get randomly sampled effects
			for bs_iter, random_snp_index in enumerate(component_random_snp_indices):
				# Randomly draw mean of effect size for given random snp_index
				effect_size_mean = cafeh_mu[tt][kk, random_snp_index]
				effect_size_var = cafeh_mu_var[tt][kk, random_snp_index]
				random_effect_size = np.random.normal(loc=effect_size_mean, scale=np.sqrt(effect_size_var))
				# Randomly draw activity factor
				random_p_active = np.random.choice([0,1], p=[1.0 - cafeh_p_active[tt,kk], cafeh_p_active[tt,kk]])

				bs_alpha_effects_vec[tt][random_snp_index,bs_iter] = bs_alpha_effects_vec[tt][random_snp_index,bs_iter] + (random_effect_size*random_p_active)


	return pmces, bs_alpha_effects_vec
...
def extract_gene_tissue_pairs_and_associated_gene_models_in_window(window_start, window_end, gene_summary_file, simulated_learned_gene_models_dir, simulation_name_string, eqtl_sample_size, window_indices, simulated_gene_expression_dir, ld_mat, eqtl_type, n_bs):
	...
	for line in f:
		...
		if gene_tss >= (window_start + 100000) and gene_tss <= (window_end - 100000):
			...
			cafeh_mu = np.load(cafeh_mu_file)
			#mu_var
			...
			cafeh_mu_var = np.load(cafeh_mu_var_file)

			# Relevent info
			n_tiss = cafeh_mu.shape[0]
			n_cis_snps = cafeh_mu.shape[2]
			...
			# Get CAFEH PMCES
			cafeh_pmces, cafeh_bs_alpha_effects_vec = extract_cafeh_pmces_and_sampled_cafeh_distribution(cafeh_pi, cafeh_p_active, cafeh_mu, cafeh_mu_var, n_tiss, n_cis_snps, n_bs, p_active_thresh, max_min_pi_ratio_thresh)


			# Loop through tissues
			for tiss_iter in range(n_tiss):
				# Skip gene, tissue pairs with no gene models
				if np.array_equal(cafeh_pmces[tiss_iter,:], np.zeros(n_cis_snps)):
					continue

				# Compute variance of gene
				if eqtl_type == 'susie':
					gene_variance = np.dot(np.dot(cafeh_pmces[tiss_iter,:], ld_mat[cis_snp_indices[window_indices],:][:,cis_snp_indices[window_indices]]), cafeh_pmces[tiss_iter,:])

					window_level_eqtl_effect_sizes = np.zeros(np.sum(window_indices))
					window_level_eqtl_effect_sizes[cis_snp_indices[window_indices]] = cafeh_pmces[tiss_iter,:]/np.sqrt(gene_variance)
					...
					'''
					bs_alpha_effects = cafeh_bs_alpha_effects_vec[tiss_iter]
	...
	return np.asarray(gene_tissue_pairs), bs_arr, np.asarray(gene_tss_arr), np.asarray(pmces_weights), np.asarray(gene_variances), susie_indices
...
tgfm_input_data_summary_file = simulated_tgfm_input_data_dir + simulation_name_string + '_eqtl_ss_' + str(eqtl_sample_size) + '_' + eqtl_type + '_bootstrapped_tgfm_input_data_w_cafeh_summary.txt'
...
for line in f:
	...
	gene_tissue_pairs, bs_eqtl_arr, gene_tissue_pairs_tss, pmces_weights, gene_variances, gene_susie_indices = extract_gene_tissue_pairs_and_associated_gene_models_in_window(window_start, window_end, gene_summary_file, simulated_learned_gene_models_dir, simulation_name_string,  eqtl_sample_size, window_indices, simulated_gene_expression_dir,ld_mat, eqtl_type, n_bs)
	...
	for bs_iter in range(n_bs):
		...
		for gene_itera, bs_eqtl_tuple in enumerate(bs_eqtl_arr):
			...
			if np.isnan(eqtl_gene_window[0]): # This occurs because of cafeh activation probabilities
				continue
	...
	window_pickle_output_file = simulated_tgfm_input_data_dir + simulation_name_string + '_' + window_name + '_eqtl_ss_' + str(eqtl_sample_size)+ '_' + eqtl_type + '_tgfm_input_data_w_cafeh.pkl'
