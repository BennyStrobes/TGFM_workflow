...
def create_mapping_from_rsid_to_gwas_beta_and_beta_se(gwas_sum_stats_file):
	dicti = {}
	...
	for line in f:
		...
		rsid = data[1]
		beta = float(data[6])
		beta_se = np.sqrt(float(data[7]))
		if rsid in dicti:
			print('assumtpiero erroror')
			pdb.set_trace()
		dicti[rsid] = (beta, beta_se)
	f.close()


	return dicti

def load_in_window_gwas_betas_and_ses_v2(rs_id_to_gwas_beta_and_beta_se, window_rsids):
	betas = []
	beta_ses = []

	for rsid in window_rsids:
		beta = rs_id_to_gwas_beta_and_beta_se[rsid][0]
		beta_se = rs_id_to_gwas_beta_and_beta_se[rsid][1]
		betas.append(beta)
		beta_ses.append(beta_se)
	return np.asarray(betas), np.asarray(beta_ses)
...
# Create dictionary mapping from rsid to gwas beta and gwas beta_se
gwas_sum_stats_file = simulated_gwas_dir + simulation_name_string +'_merged_gwas_summary_stats.txt'
...
for line in f:
	...
	gwas_beta, gwas_beta_se = load_in_window_gwas_betas_and_ses_v2(rs_id_to_gwas_beta_and_beta_se, window_rsids)
