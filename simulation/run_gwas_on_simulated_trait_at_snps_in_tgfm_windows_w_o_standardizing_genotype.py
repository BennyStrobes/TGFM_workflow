...
def run_gwas_on_specified_rsids(hapmap3_rsids, trait_values_file, gwas_plink_stem, gwas_output_file, batch_size=500):
	...
	t.write('rsid\tbeta\tbeta_se\tz\taf\n')
	...
	for snp_iter, snp_rsid in enumerate(ordered_rsids):
		...
		if np.mod(len(batch_rsids), batch_size) == 0:
			...
			for batch_iter, batch_rsid in enumerate(np.asarray(batch_rsids)):
				...
				af = np.sum(std_variant_genotype)/(2.0*len(std_variant_genotype))

				# mean center genotype
				std_variant_genotype = (std_variant_genotype - np.mean(std_variant_genotype))
				...
				t.write(batch_rsid + '\t' + str(effect_size) + '\t' + str(effect_size_se) + '\t' + str(effect_size_z) + '\t' + str(af) + '\n')
...
for line in g:
	...
	gwas_output_file = simulated_gwas_dir + simulation_name_string + '_simualated_gwas_results_unstandardized_geno_window_' + window_name + '.txt'
