	...
	for eqtl_sample_size in eqtl_sample_sizes:
		...
		for simulation_run in simulation_runs:
			...
			for tissue_iter in range(10):
				...
				bootstrapped_coefficients = np.asarray(expr_data[tissue_iter,-1].split(';')).astype(float)
				bootstrapped_coefficients[bootstrapped_coefficients < 1e-15] = 0
				if np.std(bootstrapped_coefficients, ddof=1) == 0.0:
					if np.sum(bootstrapped_coefficients != 0) > 0:
...
def create_file_containing_two_step_coloc_cs_calibration_per_high_pip_snp(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_two_step_tgfm_results_dir,simulated_coloc_results_dir, pip_threshold, coloc_cs_coverage_per_high_pip_snp_output_file, two_step_tissue_method='best'):
	# Open output file handle
	t = open(coloc_cs_coverage_per_high_pip_snp_output_file,'w')
	t.write('eQTL_sample_size\tsimulation_number\twindow_name\tgenetic_element_name\tgenetic_element_class\tcausal_genetic_element_in_cs\tPIP\n')
	...
	for simulation_number in simulation_runs:
		...
		for eqtl_sample_size in eqtl_sample_sizes:
			...
			if two_step_tissue_method == 'known_single_causal_tissue':
				best_tissue_arr = np.asarray(['tissue0'])
...
def create_file_containing_coloc_cs_calibration_per_high_pip_snp_when_limiting_to_at_most_1_causal_gt_pair_per_locus(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_coloc_results_dir, pip_threshold, coloc_cs_coverage_per_high_pip_snp_output_file, simulated_gene_position_file):
	# Load in gene tss
	gene_name_to_tss = extract_gene_tss_info(simulated_gene_position_file)
	...
	for simulation_number in simulation_runs:
		...
		causal_gene_tss = extract_causal_gene_tss(causal_genes, gene_name_to_tss)
...
def create_file_containing_focus_tg_cs_calibration_per_high_pip_snp_when_limiting_to_at_most_1_causal_gt_pair_per_locus(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_focus_results_dir, pip_threshold, focus_cs_coverage_per_high_pip_snp_output_file, simulated_gene_position_file, intercept=False):
	# Load in gene tss
	gene_name_to_tss = extract_gene_tss_info(simulated_gene_position_file)
...
def create_file_containing_two_step_focus_cs_calibration_per_high_pip_snp(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_two_step_tgfm_results_dir, simulated_focus_results_dir, pip_threshold, focus_cs_coverage_per_high_pip_snp_output_file, intercept=False, two_step_tissue_method='best'):
	# Open output file handle
	t = open(focus_cs_coverage_per_high_pip_snp_output_file,'w')
	t.write('eQTL_sample_size\tsimulation_number\twindow_name\tgenetic_element_name\tgenetic_element_class\tcausal_genetic_element_in_cs\tPIP\n')
...
def create_file_containing_focus_cs_calibration_per_high_pip_snp_when_limiting_to_at_most_1_causal_gt_pair_per_locus(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_focus_results_dir, pip_threshold, focus_cs_coverage_per_high_pip_snp_output_file, simulated_gene_position_file, intercept=False):
	# Load in gene tss
	gene_name_to_tss = extract_gene_tss_info(simulated_gene_position_file)
...
def create_file_containing_tgfm_cs_calibration_per_high_pip_snp_non_mediated_variants_include_gene_variants_limit_to_1_causal_gt(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, ln_pi_methods, twas_methods, simulated_trait_dir, simulated_tgfm_results_dir, pip_threshold, cs_coverage_per_high_pip_snp_output_file, simulated_gene_expression_dir, n_samp='100', gene_type='component_gene'):
	# Open output file handle
	t = open(cs_coverage_per_high_pip_snp_output_file,'w')
...
def create_file_containing_ctwas_cs_calibration_per_high_pip_snp_when_limiting_to_at_most_1_causal_gt_pair_per_locus(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_ctwas_results_dir, pip_threshold, cs_coverage_per_high_pip_snp_output_file, simulated_learned_gene_models_dir, simulated_gene_position_file, gene_type='component_gene'):
	# Load in gene tss
	gene_name_to_tss = extract_gene_tss_info(simulated_gene_position_file)
...
def create_file_containing_ctwas_tg_cs_calibration_per_high_pip_snp_when_limiting_to_at_most_1_causal_gt_pair_per_locus(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_ctwas_results_dir, pip_threshold, cs_coverage_per_high_pip_snp_output_file, simulated_learned_gene_models_dir,simulated_gene_position_file, gene_type='component_gene'):
	# Load in gene tss
	gene_name_to_tss = extract_gene_tss_info(simulated_gene_position_file)
...
def extract_gene_tss_info(simulated_gene_position_file):
	gene_names = []
	head_count = 0
	gene_tss = []
	gene_to_tss = {}
	...
	for line in f:
		...
		gene_name = data[1]
		tss = int(data[2])
		gene_to_tss[gene_name] = tss
	return gene_to_tss


def extract_causal_gene_tss(causal_genes, gene_name_to_tss):
	tss = []
	for gene in causal_genes:
		tss.append(gene_name_to_tss[gene.split('_')[0]])
	return np.asarray(tss)


def create_file_containing_tgfm_cs_calibration_per_high_pip_snp_when_limiting_to_at_most_1_causal_gt_pair_per_locus(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, ln_pi_methods, twas_methods, simulated_trait_dir, simulated_tgfm_results_dir, pip_threshold, cs_coverage_per_high_pip_snp_output_file, simulated_gene_position_file, n_samp='100', gene_type='component_gene'):
	# Load in gene tss
	gene_name_to_tss = extract_gene_tss_info(simulated_gene_position_file)
...
def create_file_containing_two_step_tgfm_cs_calibration_per_high_pip_snp(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_tgfm_results_dir, pip_threshold, cs_coverage_per_high_pip_snp_output_file, n_samp='100', gene_type='component_gene', two_step_tissue_methods=['best_tissue']):
	# Open output file handle
	t = open(cs_coverage_per_high_pip_snp_output_file,'w')
	...
	for simulation_number in simulation_runs:
		...
		for eqtl_sample_size in eqtl_sample_sizes:
			...
			for two_step_tissue_method in two_step_tissue_methods:
				...
				elif two_step_tissue_method == 'known_single_causal_tissue':
					tissue_vec = np.asarray(['tissue0'])
				else:
...
def create_file_containing_focus_tg_high_pip_snp_power_per_component_when_limiting_to_at_most_1_causal_gt_pair_per_locus(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_focus_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, focus_cs_power_per_component_output_file, pip_threshold, global_window_file, intercept=False):
	all_variants, all_variants_positions = extract_all_variants_and_their_positions(bim_file)
	all_genes, all_genes_positions = extract_all_genes_and_their_positions(simulated_gene_position_file)
	...
	for simulation_number in simulation_runs:
		...
		causal_gene_tss = extract_causal_gene_tss(causal_gene, gene_name_to_tss)
...
def create_file_containing_focus_high_pip_snp_power_per_component_when_limiting_to_at_most_1_causal_gt_pair_per_locus(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_focus_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, focus_cs_power_per_component_output_file, pip_threshold, global_window_file, intercept=False):
	all_variants, all_variants_positions = extract_all_variants_and_their_positions(bim_file)
	all_genes, all_genes_positions = extract_all_genes_and_their_positions(simulated_gene_position_file)
...
def create_file_containing_two_step_coloc_high_pip_snp_power_per_component(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_two_step_tgfm_results_dir, simulated_focus_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, focus_cs_power_per_component_output_file, pip_threshold, global_window_file, two_step_tissue_method='best'):
	all_variants, all_variants_positions = extract_all_variants_and_their_positions(bim_file)
	all_genes, all_genes_positions = extract_all_genes_and_their_positions(simulated_gene_position_file)
...
def count_number_of_loci_with_more_than_1_gt_pair(global_simulation_name_string, simulation_runs, simulated_trait_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, global_window_file):
	all_variants, all_variants_positions = extract_all_variants_and_their_positions(bim_file)
	all_genes, all_genes_positions = extract_all_genes_and_their_positions(simulated_gene_position_file)
	...
	n_causal_gt_pairs_per_locus_arr = []
	...
	for simulation_number in simulation_runs:
		...
		for line in f:
			...
			n_causal_gt_pairs_in_loci = 0
			for gene_name_stem in middle_genes:
				for tissue_iter in range(10):
					gene_name = gene_name_stem + '_tissue' + str(tissue_iter)
					if gene_name not in causal_genetic_elements:
						continue
					n_causal_gt_pairs_in_loci = n_causal_gt_pairs_in_loci + 1
			n_causal_gt_pairs_per_locus_arr.append(n_causal_gt_pairs_in_loci)
		f.close()

	n_causal_gt_pairs_per_locus_arr = np.asarray(n_causal_gt_pairs_per_locus_arr)
	return



def create_file_containing_coloc_high_pip_snp_power_per_component_when_limiting_to_at_most_1_causal_gt_pair_per_locus(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_focus_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, focus_cs_power_per_component_output_file, pip_threshold, global_window_file):
	all_variants, all_variants_positions = extract_all_variants_and_their_positions(bim_file)
	all_genes, all_genes_positions = extract_all_genes_and_their_positions(simulated_gene_position_file)
...
def create_file_containing_smr_gene_tissue_fdr_power_curve_data(global_simulation_name_string, eqtl_sample_size, simulation_runs, simulated_trait_dir, simulated_smr_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, fdr_power_curve_raw_data_output_file, fdr_power_curve_data_output_file, global_window_file, simulated_learned_gene_models_dir, heidi_filter=False):
	all_variants, all_variants_positions = extract_all_variants_and_their_positions(bim_file)
	all_genes, all_genes_positions = extract_all_genes_and_their_positions(simulated_gene_position_file)
	...
	for simulation_number in simulation_runs:
		...
		discovered_dicti = {}
		smr_results_file = simulated_smr_results_dir + 'simulation_' + str(simulation_number) + '_' + global_simulation_name_string + '_eqtl_ss_' + str(eqtl_sample_size) + '_smr_res.smr'
		f = open(smr_results_file)
		head_count = 0
		twice_genes = {}
		for line in f:
			line = line.rstrip()
			...
			gene_id = data[0]
			region = 'NA'
			negative_p = -float(data[18])
			if gene_id in discovered_dicti:
				print('assumption eroror')
				pdb.set_trace()
			if heidi_filter == True:
				if data[19] == 'NA':
					negative_p = -1.0
				else:
					p_heidi = float(data[19])
					if p_heidi > 0.05:
						negative_p = -1.0
			discovered_dicti[gene_id] = negative_p
		f.close()
	...
	prev_fdr = 0.0
	prev_power = 0.0
	for pip_threshold in np.hstack((np.arange(-1.0,-.01,.01), np.sort(np.unique(np.sort(pips)[np.sort(pips) > -.01])))):
		if np.sum(pips >= pip_threshold) == 0:
			continue
		fdr = 1.0 - np.sum(labels[pips >= pip_threshold] == 'causal')/np.sum(pips >= pip_threshold)
		power = np.sum(labels[pips >= pip_threshold] == 'causal')/np.sum(labels=='causal')
...
def create_file_containing_jlim_gene_tissue_fdr_power_curve_data(global_simulation_name_string, eqtl_sample_size, simulation_runs, simulated_trait_dir, simulated_jlim_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, fdr_power_curve_raw_data_output_file, fdr_power_curve_data_output_file, global_window_file, simulated_learned_gene_models_dir):
	all_variants, all_variants_positions = extract_all_variants_and_their_positions(bim_file)
	all_genes, all_genes_positions = extract_all_genes_and_their_positions(simulated_gene_position_file)
	...
	for simulation_number in simulation_runs:
		...
		discovered_dicti = {}
		jlim_results_file = simulated_jlim_results_dir + 'simulation_' + str(simulation_number) + '_' + global_simulation_name_string + '_' + str(eqtl_sample_size) + '_jlim_results.txt'
		f = open(jlim_results_file)
		head_count = 0
		twice_genes = {}
		for line in f:
			line = line.rstrip()
			...
			if data[1] == 'NA':
				continue
			negative_p = -float(data[1])
			if gene_id in discovered_dicti:
				twice_genes[gene_id.split('_')[0]] = 1
				if discovered_dicti[gene_id] != negative_p:
					pdb.set_trace()
			discovered_dicti[gene_id] = negative_p
		f.close()
...
def create_file_containing_tgfm_gene_tissue_fdr_power_curve_data(global_simulation_name_string, simulation_runs, simulated_trait_dir, simulated_tgfm_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, fdr_power_curve_raw_data_output_file, fdr_power_curve_data_output_file, twas_method, ln_pi_method, global_window_file,eqtl_sample_size, gene_type='component_gene', n_samp='100'):
	...
	for simulation_number in simulation_runs:
		...
		for line in f:
			...
			middle_alpha_pips = tgfm_res['expected_alpha_pips'][tgfm_res['middle_gene_indices']]
			middle_genes = tgfm_res['genes'][tgfm_res['middle_gene_indices']]

			for gene_iter, gene_name in enumerate(middle_genes):
				gene_pip = middle_alpha_pips[gene_iter]
				if gene_name in discovered_dicti:
					if discovered_dicti[gene_name] != gene_pip:
						print('assumption eoror')
				else:
					discovered_dicti[gene_name] = gene_pip
...
def create_file_containing_ctwas_tg_high_pip_snp_power_per_component_when_limiting_to_at_most_1_causal_gt_pair_per_locus(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_ctwas_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, cs_power_per_component_output_file, pip_threshold, global_window_file, simulated_learned_gene_models_dir, gene_type='component_gene'):
	all_variants, all_variants_positions = extract_all_variants_and_their_positions(bim_file)
	all_genes, all_genes_positions = extract_all_genes_and_their_positions(simulated_gene_position_file)
...
def create_file_containing_tgfm_high_pip_snp_power_per_component_when_limiting_to_at_most_1_causal_gt_pair_per_locus(global_simulation_name_string, eqtl_sample_sizes, simulation_runs, ln_pi_methods, simulated_trait_dir, simulated_tgfm_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, cs_power_per_component_output_file, pip_threshold, twas_methods, global_window_file, n_samp='100', gene_type='component_gene'):
	all_variants, all_variants_positions = extract_all_variants_and_their_positions(bim_file)
	all_genes, all_genes_positions = extract_all_genes_and_their_positions(simulated_gene_position_file)
...
simulated_jlim_results_dir = sys.argv[19]
simulated_smr_results_dir = sys.argv[20]
...
for model_name in model_names:
	...
	type_1_error_output_file =  simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_' + model_name + '_h2_type_1_error_gaussian_approximation2.txt'
	...
	power_output_file =  simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_' + model_name + '_mediated_h2_power_gaussian_approximation2.txt'
...
for pip_threshold in pip_thresholds:
	for n_posterior_samples in n_posterior_samples_vec:
		print(pip_threshold)
		# Create file with one line per cs (columns: eQTL_sample_size, ln_pvalue_method, simulation_num, window_num, component_num, boolean_causal_variant_in_cs)
		cs_coverage_per_high_pip_snp_output_file =  simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold) + '_eqtl_ss_500_n_samp_' + str(n_posterior_samples) + '_nm_variant_alt_calibration_per_component.txt'
		create_file_containing_tgfm_cs_calibration_per_high_pip_snp_non_mediated_variants_include_gene_variants(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, ln_pi_methods, twas_methods, simulated_trait_dir, simulated_tgfm_results_dir, pip_threshold, cs_coverage_per_high_pip_snp_output_file, simulated_gene_expression_dir, n_samp=n_posterior_samples)
		cs_high_pip_coverage_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold)+ '_eqtl_ss_500_n_samp_' + str(n_posterior_samples) + '_nm_variant_alt_calibration.txt'
		create_file_containing_averaged_tgfm_high_pip_calibration(cs_coverage_per_high_pip_snp_output_file, cs_high_pip_coverage_output_file, eqtl_sample_sizes, ln_pi_methods, twas_methods)
		print(cs_high_pip_coverage_output_file)
...
for pip_threshold in pip_thresholds:
	for n_posterior_samples in n_posterior_samples_vec:
		print(pip_threshold)
		# Create file with one line per causal element (columns: eQTL_sample_size, ln_pvalue_method, simulation_num, window_num, gene_or_variant, causal_element_name, boolean_causal_element_in_cs)
		cs_power_per_component_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold)+ '_eqtl_ss_500_n_samp_' + str(n_posterior_samples) + '_power_per_component.txt'
		create_file_containing_tgfm_high_pip_snp_power_per_component(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, ln_pi_methods, simulated_trait_dir, simulated_tgfm_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, cs_power_per_component_output_file, pip_threshold, twas_methods, global_window_file, n_samp=n_posterior_samples)
		cs_power_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold) + '_eqtl_ss_500_n_samp_' + str(n_posterior_samples)+ '_power.txt'
		create_file_containing_averaged_tgfm_cs_power(cs_power_per_component_output_file, cs_power_output_file, eqtl_sample_sizes, ln_pi_methods,twas_methods)
		print(cs_power_output_file)

##################################
# Vary n posterior samples
# TGFM (Gene) Coverage/Calibration to detect snps with PIP > threshold
##################################
# Vary ln_pi_method
# Used eQTL sample sizes
eqtl_sample_sizes = np.asarray(['500' ])
...
for pip_threshold in pip_thresholds:
	for n_posterior_samples in n_posterior_samples_vec:
		# Create file with one line per cs (columns: eQTL_sample_size, ln_pvalue_method, simulation_num, window_num, component_num, boolean_causal_variant_in_cs)
		cs_coverage_per_high_pip_snp_output_file =  simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold)+ '_eqtl_ss_500_n_samp_' + str(n_posterior_samples) + '_gene_calibration_per_component.txt'
		create_file_containing_tgfm_cs_calibration_per_high_pip_genes(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, ln_pi_methods, twas_methods, simulated_trait_dir, simulated_tgfm_input_data_dir, simulated_tgfm_results_dir, pip_threshold, cs_coverage_per_high_pip_snp_output_file, n_samp=n_posterior_samples)
		cs_high_pip_coverage_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold)+ '_eqtl_ss_500_n_samp_' + str(n_posterior_samples) + '_gene_calibration.txt'
		create_file_containing_averaged_tgfm_high_pip_calibration(cs_coverage_per_high_pip_snp_output_file, cs_high_pip_coverage_output_file, eqtl_sample_sizes, ln_pi_methods, twas_methods)
		print(cs_high_pip_coverage_output_file)

##################################
# Vary n posterior samples
# TGFM (Gene) power to detect snps with PIP > threshold
##################################
# Vary ln_pi_method
# Used eQTL sample sizes
eqtl_sample_sizes = np.asarray(['500' ])
...
for pip_threshold in pip_thresholds:
	for n_posterior_samples in n_posterior_samples_vec:
		cs_power_per_component_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold) + '_eqtl_ss_500_n_samp_' + str(n_posterior_samples)+ '_gene_power_per_component.txt'
		create_file_containing_tgfm_high_pip_gene_power_per_component(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, ln_pi_methods, simulated_trait_dir, simulated_tgfm_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, cs_power_per_component_output_file, pip_threshold, twas_methods, global_window_file, n_samp=n_posterior_samples)
		cs_power_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold) + '_eqtl_ss_500_n_samp_' + str(n_posterior_samples)+ '_gene_power.txt'
		create_file_containing_averaged_tgfm_cs_power(cs_power_per_component_output_file, cs_power_output_file, eqtl_sample_sizes, ln_pi_methods,twas_methods)
		print(cs_power_output_file)
'''

##################################
# Vary scaling factors on prior variances
# TGFM Coverage/Calibration to detect snps with PIP > threshold
# Non-mediated variants also include expression-mediated variants
##################################
...
# (nm variant prior scale, non-causal gt prior scale)
scaling_factors = [('0.5', '1.0'), ('2.0', '1.0'), ('1.0', '0.5'), ('1.0', '2.0')]
'''
for pip_threshold in pip_thresholds:
	for scaling_factor in scaling_factors:
		print(scaling_factor)
		# Extract scaling factors
		nm_var_scale_factor = scaling_factor[0]
		gt_scale_factor = scaling_factor[1]
		ln_pi_methods = np.asarray(['bootstrapped_scaled_nm_' + nm_var_scale_factor + '_gt_' + gt_scale_factor])

		# Create file with one line per cs (columns: eQTL_sample_size, ln_pvalue_method, simulation_num, window_num, component_num, boolean_causal_variant_in_cs)
		cs_coverage_per_high_pip_snp_output_file =  simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold) + '_eqtl_ss_500_scaled_nm_'+ nm_var_scale_factor + '_gt_' + gt_scale_factor + '_nm_variant_alt_calibration_per_component.txt'
		create_file_containing_tgfm_cs_calibration_per_high_pip_snp_non_mediated_variants_include_gene_variants(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, ln_pi_methods, twas_methods, simulated_trait_dir, simulated_tgfm_results_dir, pip_threshold, cs_coverage_per_high_pip_snp_output_file, simulated_gene_expression_dir)
		cs_high_pip_coverage_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold)+ '_eqtl_ss_500_scaled_nm_' + nm_var_scale_factor + '_gt_' + gt_scale_factor + '_nm_variant_alt_calibration.txt'
		create_file_containing_averaged_tgfm_high_pip_calibration(cs_coverage_per_high_pip_snp_output_file, cs_high_pip_coverage_output_file, eqtl_sample_sizes, ln_pi_methods, twas_methods)
		print(cs_high_pip_coverage_output_file)
...
# (nm variant prior scale, non-causal gt prior scale)
scaling_factors = [('0.5', '1.0'), ('2.0', '1.0'), ('1.0', '0.5'), ('1.0', '2.0')]
scaling_factors = [('1.0', '0.5'), ('1.0', '2.0')]
...
for pip_threshold in pip_thresholds:
	for scaling_factor in scaling_factors:
		...
		# Create file with one line per causal element (columns: eQTL_sample_size, ln_pvalue_method, simulation_num, window_num, gene_or_variant, causal_element_name, boolean_causal_element_in_cs)
		cs_power_per_component_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold)+ '_eqtl_ss_500_scaled_nm_'+ nm_var_scale_factor + '_gt_' + gt_scale_factor +'_power_per_component.txt'
		create_file_containing_tgfm_high_pip_snp_power_per_component(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, ln_pi_methods, simulated_trait_dir, simulated_tgfm_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, cs_power_per_component_output_file, pip_threshold, twas_methods, global_window_file)
		cs_power_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold) + '_eqtl_ss_500_scaled_nm_'+ nm_var_scale_factor + '_gt_' + gt_scale_factor + '_power.txt'
		create_file_containing_averaged_tgfm_cs_power(cs_power_per_component_output_file, cs_power_output_file, eqtl_sample_sizes, ln_pi_methods,twas_methods)
		print(cs_power_output_file)
...
for pip_threshold in pip_thresholds:
	for scaling_factor in scaling_factors:
		...
		# Create file with one line per cs (columns: eQTL_sample_size, ln_pvalue_method, simulation_num, window_num, component_num, boolean_causal_variant_in_cs)
		cs_coverage_per_high_pip_snp_output_file =  simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold)+ '_eqtl_ss_500_scaled_nm_'+ nm_var_scale_factor + '_gt_' + gt_scale_factor + '_gene_calibration_per_component.txt'
		create_file_containing_tgfm_cs_calibration_per_high_pip_genes(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, ln_pi_methods, twas_methods, simulated_trait_dir, simulated_tgfm_input_data_dir, simulated_tgfm_results_dir, pip_threshold, cs_coverage_per_high_pip_snp_output_file)
		cs_high_pip_coverage_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold)+ '_eqtl_ss_500_scaled_nm_'+ nm_var_scale_factor + '_gt_' + gt_scale_factor +  '_gene_calibration.txt'
		create_file_containing_averaged_tgfm_high_pip_calibration(cs_coverage_per_high_pip_snp_output_file, cs_high_pip_coverage_output_file, eqtl_sample_sizes, ln_pi_methods, twas_methods)
		print(cs_high_pip_coverage_output_file)
...
for pip_threshold in pip_thresholds:
	for scaling_factor in scaling_factors:
		...
		cs_power_per_component_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold) + '_eqtl_ss_500_scaled_nm_'+ nm_var_scale_factor + '_gt_' + gt_scale_factor + '_gene_power_per_component.txt'
		create_file_containing_tgfm_high_pip_gene_power_per_component(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, ln_pi_methods, simulated_trait_dir, simulated_tgfm_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, cs_power_per_component_output_file, pip_threshold, twas_methods, global_window_file)
		cs_power_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_tgfm_pip_' + str(pip_threshold) + '_eqtl_ss_500_scaled_nm_'+ nm_var_scale_factor + '_gt_' + gt_scale_factor + '_gene_power.txt'
		create_file_containing_averaged_tgfm_cs_power(cs_power_per_component_output_file, cs_power_output_file, eqtl_sample_sizes, ln_pi_methods,twas_methods)
		print(cs_power_output_file)
'''
...
##################################
# JLIM FDR-power curve
##################################
'''
tmp_simulation_runs = np.arange(1,51)
bads = {}
...
eqtl_sample_sizes = ['realistic', '300', '500' ,'1000']
for eqtl_ss in eqtl_sample_sizes:
	fdr_power_curve_raw_data_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_' + 'jlim' + '_' + eqtl_ss + '_fdr_power_curve_raw_data.txt'
	fdr_power_curve_data_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_' + 'jlim' + '_' + eqtl_ss + '_fdr_power_curve_data.txt'
	create_file_containing_jlim_gene_tissue_fdr_power_curve_data(local_simulation_name_string, eqtl_ss, simulation_runs, simulated_trait_dir, simulated_jlim_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, fdr_power_curve_raw_data_output_file,fdr_power_curve_data_output_file, global_window_file, simulated_learned_gene_models_dir)
	print(fdr_power_curve_data_output_file)
'''
'''
##################################
...
eqtl_sample_sizes = ['realistic', '300', '500', '1000']
for eqtl_ss in eqtl_sample_sizes:
	fdr_power_curve_raw_data_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_' + 'smr' + '_' + eqtl_ss + '_fdr_power_curve_raw_data.txt'
	fdr_power_curve_data_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_' + 'smr' + '_' + eqtl_ss + '_fdr_power_curve_data.txt'
	create_file_containing_smr_gene_tissue_fdr_power_curve_data(local_simulation_name_string, eqtl_ss, simulation_runs, simulated_trait_dir, simulated_smr_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, fdr_power_curve_raw_data_output_file,fdr_power_curve_data_output_file, global_window_file, simulated_learned_gene_models_dir)

	print(fdr_power_curve_data_output_file)


	fdr_power_curve_raw_data_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_' + 'smr_heidi' + '_' + eqtl_ss + '_fdr_power_curve_raw_data.txt'
	fdr_power_curve_data_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_' + 'smr_heidi' + '_' + eqtl_ss + '_fdr_power_curve_data.txt'
	create_file_containing_smr_gene_tissue_fdr_power_curve_data(local_simulation_name_string, eqtl_ss, simulation_runs, simulated_trait_dir, simulated_smr_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, fdr_power_curve_raw_data_output_file,fdr_power_curve_data_output_file, global_window_file, simulated_learned_gene_models_dir, heidi_filter=True)
...
#############################################################
# pleiotropy Fine-mapping simulation
#############################################################
gene_trait_architecture = '2_caus_t'
...
#############################################################
# 1 causal tissue fine-mapping simulation
#############################################################
...
##################################
# Special-two step TGFM Coverage/Calibration to detect snps with PIP > threshold
# Where causal tissue is known
##################################
pip_thresholds = [.3, .5, .7, .9, .95, .99]
# Vary ln_pi_method
for pip_threshold in pip_thresholds:
	print(pip_threshold)
	# Create file with one line per cs (columns: eQTL_sample_size, ln_pvalue_method, simulation_num, window_num, component_num, boolean_causal_variant_in_cs)
	cs_coverage_per_high_pip_snp_output_file =  simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_tgfm_pip_' + str(pip_threshold) + '_calibration_per_component.txt'
	create_file_containing_two_step_tgfm_cs_calibration_per_high_pip_snp(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_two_step_tgfm_results_dir, pip_threshold, cs_coverage_per_high_pip_snp_output_file, two_step_tissue_methods=['known_single_causal_tissue'])

	cs_high_pip_coverage_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_tgfm_pip_' + str(pip_threshold) + '_calibration.txt'
	create_file_containing_averaged_tgfm_high_pip_calibration_two_step(cs_coverage_per_high_pip_snp_output_file, cs_high_pip_coverage_output_file, eqtl_sample_sizes, ['susie_sampler'], ['iterative'], ['known_single_causal_tissue'])
	print(cs_high_pip_coverage_output_file)
...
##################################
# Special-two step TGFM power to detect gene-tissue pairs with PIP > threshold
# Where causal tissue is known
##################################
pip_thresholds = [.3, .5, .7, .9, .95, .99]
for pip_threshold in pip_thresholds:
	print(pip_threshold)
	# Create file with one line per causal element (columns: eQTL_sample_size, ln_pvalue_method, simulation_num, window_num, gene_or_variant, causal_element_name, boolean_causal_element_in_cs)
	cs_power_per_component_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_tgfm_pip_' + str(pip_threshold) + '_power_per_component.txt'
	create_file_containing_two_step_tgfm_high_pip_snp_power_per_component(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, ln_pi_methods, simulated_trait_dir, simulated_two_step_tgfm_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, cs_power_per_component_output_file, pip_threshold, twas_methods, global_window_file, two_step_tissue_method='known_single_causal_tissue')

	cs_power_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_tgfm_pip_' + str(pip_threshold) + '_power.txt'
	create_file_containing_averaged_tgfm_cs_power(cs_power_per_component_output_file, cs_power_output_file, eqtl_sample_sizes, ln_pi_methods,twas_methods)
	print(cs_power_output_file)
...
##################################
# Special Two-step cTWAS Coverage/Calibration to detect snps with PIP > threshold
# Where causal tissue is known
##################################
for pip_threshold in pip_thresholds:
	print(pip_threshold)
	# Create file with one line per cs (columns: eQTL_sample_size, ln_pvalue_method, simulation_num, window_num, component_num, boolean_causal_variant_in_cs)
	cs_coverage_per_high_pip_snp_output_file =  simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_ctwas_pip_' + str(pip_threshold) + '_calibration_per_component.txt'
	create_file_containing_two_step_ctwas_cs_calibration_per_high_pip_snp(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_two_step_tgfm_results_dir, simulated_causal_twas_results_dir, pip_threshold, cs_coverage_per_high_pip_snp_output_file, simulated_learned_gene_models_dir, two_step_tissue_method='known_single_causal_tissue')
	cs_high_pip_coverage_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_ctwas_pip_' + str(pip_threshold) + '_calibration.txt'
	create_file_containing_averaged_tgfm_high_pip_calibration(cs_coverage_per_high_pip_snp_output_file, cs_high_pip_coverage_output_file, eqtl_sample_sizes, ['ctwas'], ['lasso'], agg_eqtl_ss=False)
	print(cs_high_pip_coverage_output_file)
...
##################################
# Special two-step cTWAS Power to detect gene-tissue pairs with PIP > threshold
# Where causal tissue is known
##################################
pip_thresholds = [.3, .5, .7, .9, .95, .99]
gene_type='component_gene'
for pip_threshold in pip_thresholds:
	print(pip_threshold)
	# Create file with one line per causal element (columns: eQTL_sample_size, ln_pvalue_method, simulation_num, window_num, gene_or_variant, causal_element_name, boolean_causal_element_in_cs)
	cs_power_per_component_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_ctwas_pip_' + str(pip_threshold) + '_power_per_component.txt'
	create_file_containing_two_step_ctwas_high_pip_snp_power_per_component(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_two_step_tgfm_results_dir, simulated_causal_twas_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, cs_power_per_component_output_file, pip_threshold, global_window_file,simulated_learned_gene_models_dir, two_step_tissue_method='known_single_causal_tissue')
	cs_power_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_ctwas_pip_' + str(pip_threshold) + '_power.txt'
	create_file_containing_averaged_tgfm_cs_power(cs_power_per_component_output_file, cs_power_output_file, eqtl_sample_sizes, ['ctwas'],['lasso'], agg_eqtl_ss=False)
	print(cs_power_output_file)
...
##################################
# Special Two-step FOCUS Coverage/Calibration to detect snps with PIP > threshold
# Where causal tissue is known
##################################
pip_thresholds = [.3, .5, .7, .9, .95, .99]
# Vary ln_pi_method
for pip_threshold in pip_thresholds:
	print(pip_threshold)
	# Calibration to detect causal gene-tissue pairs (no genotype intercept)
	focus_cs_coverage_per_high_pip_snp_output_file =  simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_focus_pip_' + str(pip_threshold) + '_calibration_per_component.txt'
	create_file_containing_two_step_focus_cs_calibration_per_high_pip_snp(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir,simulated_two_step_tgfm_results_dir, simulated_focus_results_dir, pip_threshold, focus_cs_coverage_per_high_pip_snp_output_file, two_step_tissue_method='known_single_causal_tissue')
	focus_cs_high_pip_coverage_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_focus_pip_' + str(pip_threshold) + '_calibration.txt'
	create_file_containing_averaged_focus_high_pip_calibration(focus_cs_coverage_per_high_pip_snp_output_file, focus_cs_high_pip_coverage_output_file, eqtl_sample_sizes)
	print(focus_cs_high_pip_coverage_output_file)
...
##################################
# Special Two-step FOCUS Power to detect gene-tissue pairs with PIP > threshold
# Where causal tissue is known
##################################
pip_thresholds = [.3, .5, .7, .9, .95, .99]
for pip_threshold in pip_thresholds:
	print(pip_threshold)
	# Power to detect causal gene-tissue pairs (no genotype intercept)
	focus_cs_power_per_component_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_focus_pip_' + str(pip_threshold) + '_power_per_component.txt'
	create_file_containing_two_step_focus_high_pip_snp_power_per_component(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir,simulated_two_step_tgfm_results_dir, simulated_focus_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, focus_cs_power_per_component_output_file, pip_threshold, global_window_file, two_step_tissue_method='known_single_causal_tissue')
	focus_cs_power_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_focus_pip_' + str(pip_threshold) + '_power.txt'
	create_file_containing_averaged_focus_cs_power(focus_cs_power_per_component_output_file, focus_cs_power_output_file, eqtl_sample_sizes)
	print(focus_cs_power_output_file)
...
##################################
# Special Two-step coloc Coverage/Calibration to detect snps with PIP > threshold
# Where causal tissue is known
##################################
pip_thresholds = [.3, .5, .7, .9, .95, .99]
# Vary ln_pi_method
for pip_threshold in pip_thresholds:
	print(pip_threshold)
	# Create file with one line per cs (columns: eQTL_sample_size, simulation_num, window_num, boolean_causal_variant_in_cs)
	coloc_cs_coverage_per_high_pip_snp_output_file =  simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_coloc_pip_' + str(pip_threshold) + '_calibration_per_component.txt'
	create_file_containing_two_step_coloc_cs_calibration_per_high_pip_snp(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_two_step_tgfm_results_dir,simulated_coloc_results_dir, pip_threshold, coloc_cs_coverage_per_high_pip_snp_output_file, two_step_tissue_method='known_single_causal_tissue')
	coloc_cs_high_pip_coverage_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_coloc_pip_' + str(pip_threshold) + '_calibration.txt'
	create_file_containing_averaged_focus_high_pip_calibration(coloc_cs_coverage_per_high_pip_snp_output_file, coloc_cs_high_pip_coverage_output_file, eqtl_sample_sizes)
	print(coloc_cs_high_pip_coverage_output_file)
...
##################################
# Special Two-step coloc Power to detect gene-tissue pairs with PIP > threshold
# Where causal tissue is known
##################################
pip_thresholds = [.3, .5, .7, .9, .95, .99]
for pip_threshold in pip_thresholds:
	print(pip_threshold)
	# Create file with one line per causal element (columns: eQTL_sample_size, ln_pvalue_method, simulation_num, window_num, gene_or_variant, causal_element_name, boolean_causal_element_in_cs)
	coloc_cs_power_per_component_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_coloc_pip_' + str(pip_threshold) + '_power_per_component.txt'
	create_file_containing_two_step_coloc_high_pip_snp_power_per_component(local_simulation_name_string, eqtl_sample_sizes, simulation_runs, simulated_trait_dir, simulated_two_step_tgfm_results_dir, simulated_coloc_results_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, coloc_cs_power_per_component_output_file, pip_threshold, global_window_file, two_step_tissue_method='known_single_causal_tissue')
	coloc_cs_power_output_file = simulated_organized_results_dir + 'organized_simulation_' + local_simulation_name_string + '_special_two_step_known_tiss_coloc_pip_' + str(pip_threshold) + '_power.txt'
	create_file_containing_averaged_focus_cs_power(coloc_cs_power_per_component_output_file, coloc_cs_power_output_file, eqtl_sample_sizes)
	print(coloc_cs_power_output_file)
...
##################################
# Count number of loci with more than 1 gt pair
##################################
'''
count_number_of_loci_with_more_than_1_gt_pair(local_simulation_name_string, simulation_runs, simulated_trait_dir, simulated_tgfm_input_data_dir, bim_file, simulated_gene_position_file, global_window_file)
'''
